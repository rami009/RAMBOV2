...
file:close()
sleep(1)
os.execute(' screen -S rambov2 ./rambov2')
end
...
RAMBOV2_SUDO = sudos.RmOwner   -------bot owner
RAMBOV2_TOKEN = sudos.Token -----RAMBOV2_TOKEN
bot_id = sudos.Bot_ID ---id bot
RAMBOV2 = tonumber(_redis.Bot_ID)
...
local bot_id = redis:get(RAMBOV2.."Bot:RmBotAccount") or tonumber(_redis.Bot_ID)
...
redis:sadd(RAMBOV2.."Bot:RmSudos", user)
end
redis:setex(RAMBOV2.."bot:reload", 7230, true)
redis:set(RAMBOV2.."Bot:RmOwnerBot", config.RmOwner or 0)
redis:set(RAMBOV2.."Bot:Run", config.Run or 0)
local Api = config.Token:match("(%d+)")
local RD = RNM or 0
if Api then
redis:set(RAMBOV2.."Bot:Api_ID", Api)
end
function AuthoritiesEn()
local hash = "Bot:RmSudos"
local list = redis:smembers(RAMBOV2..hash)
local OwnerRm_ = redis:get(RAMBOV2.."Bot:RmOwnerBot")
local text = "List of Authorities :\n"
local user_info_ = redis:get(RAMBOV2.."user:Name" .. OwnerRm_)
...
local user_info = redis:get(RAMBOV2.."user:Name" .. v)
if user_info then
...
local list2 = redis:smembers(RAMBOV2..hash2)
if #list2 ~= 0 then
text = text .. [[
...
redis:set(RAMBOV2.."AuthoritiesEn", text)
end
...
redis:set(RAMBOV2.."AuthoritiesFa", text)
end
AuthoritiesEn()
AuthoritiesFa()
end
...
redis:del(RAMBOV2.."Bot:RmSudos")
redis:del(RAMBOV2.."Bot:RmOwnerBot")
redis:del(RAMBOV2.."Bot:Token")
redis:del(RAMBOV2.."Bot:Channel")
setdata()
end
...
usr = string.gsub(usr, "%s+$", "")
usr = string.gsub(usr, "[\n\r]+", " ")
redis:set(RAMBOV2.."Bot:ServerUser", usr)----------ServerUser
redis:del(RAMBOV2.."MatchApi")
redis:del(RAMBOV2.."Set_Our_ID")
redis:del(RAMBOV2.."Open:Chats")
local Rmdata = redis:get(RAMBOV2.."Botid" .. bot_id) or "\n"
local BotRmdata = redis:get(RAMBOV2.."RmOwnerBot" .. config.RmOwner) or "\n"
if redis:get(RAMBOV2.."Rank:Data") then
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. Rmdata .. "\027[00m")
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. BotRmdata .. "\027[00m")
for v, user in pairs(config.Sudo_Users) do
local SudoData = redis:get(RAMBOV2.."RmSudos" .. user)
if SudoData then
...
redis:set(RAMBOV2.."Our_ID", result.id_)
end
...
local lists = redis:smembers(RAMBOV2..hashs)
redis:del(RAMBOV2.."SudoNumberGp" .. user_id)
for k, v in pairs(lists) do
redis:incr(RAMBOV2.."SudoNumberGp" .. user_id)
end
end
local setnumbergp_three = function(user_id)
local hashss = "sudo:data:" .. user_id
local lists = redis:smembers(RAMBOV2..hashss)
redis:del(RAMBOV2.."SudoNumberGp" .. user_id)
for k, v in pairs(lists) do
redis:incr(RAMBOV2.."SudoNumberGp" .. user_id)
end
end
local list = redis:smembers(RAMBOV2.."Bot:Admins")
for k, v in pairs(list) do
setnumbergp_two(v)
end
local lists = redis:smembers(RAMBOV2.."Bot:RmSudos")
for k, v in pairs(lists) do
setnumbergp_three(v)
end
redis:setex(RAMBOV2.."bot:reload", 7230, true)
...
local Bot_Channel = redis:get(RAMBOV2.."Bot:Channel") or tostring(_redis.Channel)
local sudo_users = _config.Sudo_Users
local Rm_Owner = redis:get(RAMBOV2.."Bot:RmOwnerBot")
local run = redis:get(RAMBOV2.."Bot:Run") or "True"
if not redis:get(RAMBOV2.."setnumbergp") then
setnumbergp()
redis:setex(RAMBOV2.."setnumbergp", 5 * hour, true)
end
...
CH SOURCE : @RAMBOV2_ch
]].."\n\027[00m")
-----------------------function is_Rm----------by RAMBOV2------------------------
...
local var = false
if redis:sismember(RAMBOV2.."Bot:RmSudos", msg.sender_user_id_) then
var = true
end
if msg.sender_user_id_ == tonumber(Rm_Owner) then
var = true
...
local var = false
if redis:sismember(RAMBOV2.."Bot:RmSudos", user_id) then
var = true
end
if user_id == tonumber(Rm_Owner) then
var = true
...
local admin = redis:sismember(RAMBOV2..hashsb, user_id)
if admin then
var = true
end
if redis:sismember(RAMBOV2.."Bot:RmSudos", user_id) then
...
local monshi = redis:sismember(RAMBOV2..hashssk, user_id)
local hashs = "Bot:Admins"
local admin = redis:sismember(RAMBOV2..hashs, user_id)
if monshi then
var = true
end
if admin then
...
local owner = redis:sismember(RAMBOV2..hash, user_id)
if monshi then
var = true
end
if admin then
...
local momod = redis:sismember(RAMBOV2..hash, user_id)
local hashs = "Bot:Admins"
local admin = redis:sismember(RAMBOV2..hashs, user_id)
local hashssk = "bot:monshis:" .. chat_id
local monshi = redis:sismember(RAMBOV2..hashssk, user_id)
local hashss = "bot:owners:" .. chat_id
local owner = redis:sismember(RAMBOV2..hashss, user_id)
local our_id = redis:get(RAMBOV2.."Our_ID") or 0
if momod then
var = true
end
if owner then
...
local vipmem = redis:sismember(RAMBOV2..hashsss, user_id)
if vipmem then
var = true
end
if momod then
...
local vipmems = redis:sismember(RAMBOV2..hashsss, user_id)
if vipmems then
var = true
end
if redis:sismember(RAMBOV2.."Bot:RmSudos", user_id) then
...
local banned = redis:sismember(RAMBOV2..hash, user_id)
if banned then
var = true
end
return var
...
local var = false
local hash = "bot:gban:"
local gbanned = redis:sismember(RAMBOV2..hash, user_id)
if gbanned then
var = true
end
return var
...
local muted = redis:sismember(RAMBOV2..hash, user_id)
local muted2 = redis:get(RAMBOV2..hash2)
if muted then
var = true
end
if muted2 then
...
if redis:get(RAMBOV2.."GetTime") then
local data = redis:get(RAMBOV2.."GetTime")
...
redis:setex(RAMBOV2.."GetTime", 10, url)
local A = jdat.FAtime
local B = jdat.FAdate
if A and B then
local T = {time = A, date = B}
...
------------------function Join CH------------------------------------------
function sudo_JoinCh(msg)
local var = true
if redis:get(RAMBO.."Rmjoin1") then
local channel = ''..redis:get(RAMBO..'Rmch1')..''
local url , res = https.request('https://api.telegram.org/bot'..RAMBO_TOKEN..'/getchatmember?chat_id='..channel..'&user_id='..msg.sender_user_id_)
...
redis:set(RAMBOV2..'group:name'..GroupID,data.title_) end,nil) return redis:get(RAMBOV2..'group:name'..GroupID) end
...
link = redis:get(RAMBOV2.."bot:group:link" .. chat)
if link then
...
redis:del(RAMBOV2.."bot:muteall" .. chat_id)
redis:del(RAMBOV2.."bot:text:mute" .. chat_id)
redis:del(RAMBOV2.."bot:photo:mute" .. chat_id)
redis:del(RAMBOV2.."bot:video:mute" .. chat_id)
redis:del(RAMBOV2.."bot:selfvideo:mute" .. chat_id)
redis:del(RAMBOV2.."bot:gifs:mute" .. chat_id)
redis:del(RAMBOV2.."anti-flood:" .. chat_id)
redis:del(RAMBOV2.."flood:max:" .. chat_id)
redis:del(RAMBOV2.."bot:sens:spam" .. chat_id)
redis:del(RAMBOV2.."floodstatus" .. chat_id)
redis:del(RAMBOV2.."bot:music:mute" .. chat_id)
redis:del(RAMBOV2.."bot:bots:mute" .. chat_id)
redis:del(RAMBOV2.."bot:duplipost:mute" .. chat_id)
redis:del(RAMBOV2.."bot:inline:mute" .. chat_id)
redis:del(RAMBOV2.."bot:cmds" .. chat_id)
redis:del(RAMBOV2.."bot:bots:mute" .. chat_id)
redis:del(RAMBOV2.."bot:voice:mute" .. chat_id)
redis:del(RAMBOV2.."editmsg" .. chat_id)
redis:del(RAMBOV2.."bot:links:mute" .. chat_id)
redis:del(RAMBOV2.."bot:pin:mute" .. chat_id)
redis:del(RAMBOV2.."bot:sticker:mute" .. chat_id)
redis:del(RAMBOV2.."bot:tgservice:mute" .. chat_id)
redis:del(RAMBOV2.."bot:webpage:mute" .. chat_id)
redis:del(RAMBOV2.."bot:strict" .. chat_id)
redis:del(RAMBOV2.."bot:hashtag:mute" .. chat_id)
redis:del(RAMBOV2.."tags:lock" .. chat_id)
redis:del(RAMBOV2.."bot:location:mute" .. chat_id)
redis:del(RAMBOV2.."bot:contact:mute" .. chat_id)
redis:del(RAMBOV2.."bot:english:mute" .. chat_id)
redis:del(RAMBOV2.."bot:arabic:mute" .. chat_id)
redis:del(RAMBOV2.."bot:forward:mute" .. chat_id)
redis:del(RAMBOV2.."bot:member:lock" .. chat_id)
redis:del(RAMBOV2.."bot:document:mute" .. chat_id)
redis:del(RAMBOV2.."markdown:lock" .. chat_id)
redis:del(RAMBOV2.."Game:lock" .. chat_id)
redis:del(RAMBOV2.."bot:spam:mute" .. chat_id)
redis:del(RAMBOV2.."post:lock" .. chat_id)
redis:del(RAMBOV2.."bot:welcome" .. chat_id)
redis:del(RAMBOV2.."delidstatus" .. chat_id)
redis:del(RAMBOV2.."delpro:" .. chat_id)
redis:del(RAMBOV2.."sharecont" .. chat_id)
redis:del(RAMBOV2.."sayedit" .. chat_id)
redis:del(RAMBOV2.."welcome:" .. chat_id)
redis:del(RAMBOV2.."bot:group:link" .. chat_id)
redis:del(RAMBOV2.."bot:filters:" .. chat_id)
redis:del(RAMBOV2.."bot:muteall:Time" .. chat_id)
redis:del(RAMBOV2.."bot:muteall:start" .. chat_id)
redis:del(RAMBOV2.."bot:muteall:stop" .. chat_id)
redis:del(RAMBOV2.."bot:muteall:start_Unix" .. chat_id)
redis:del(RAMBOV2.."bot:muteall:stop_Unix" .. chat_id)
redis:del(RAMBOV2.."bot:muteall:Run" .. chat_id)
redis:del(RAMBOV2.."bot:muted:" .. chat_id)
end
...
redis:set(RAMBOV2.."bot:webpage:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:inline:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:bots:mute" .. chat_id, true)
redis:set(RAMBOV2.."tags:lock" .. chat_id, true)
redis:set(RAMBOV2.."markdown:lock" .. chat_id, true)
redis:set(RAMBOV2.."bot:links:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:hashtag:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:spam:mute" .. chat_id, true)
redis:set(RAMBOV2.."anti-flood:" .. chat_id, true)
redis:set(RAMBOV2.."Game:lock" .. chat_id, true)
redis:set(RAMBOV2.."bot:panel" .. chat_id, "one")
end
...
redis:set(RAMBOV2.."anti-flood:" .. chat_id, true)
redis:set(RAMBOV2.."Game:lock" .. chat_id, true)
redis:set(RAMBOV2.."post:lock" .. chat_id, true)
redis:set(RAMBOV2.."bot:forward:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:photo:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:video:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:selfvideo:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:gifs:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:sticker:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:location:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:document:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:panel" .. chat_id, "two")
end
...
redis:set(RAMBOV2.."bot:links:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:spam:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:sens:spam" .. chat_id, 500)
redis:set(RAMBOV2.."anti-flood:" .. chat_id, true)
redis:set(RAMBOV2.."Game:lock" .. chat_id, true)
redis:set(RAMBOV2.."bot:cmds" .. chat_id, true)
redis:set(RAMBOV2.."bot:duplipost:mute" .. chat_id, true)
redis:set(RAMBOV2.."bot:panel" .. chat_id, "three")
end
...
local user_id
local user = false
local list1 = redis:smembers(RAMBOV2.."Bot:RmSudos")
local list2 = redis:smembers(RAMBOV2.."Bot:Admins")
for k, v in pairs(list1) do
local hash = "sudo:data:" .. v
local is_add = redis:sismember(RAMBOV2..hash, chat)
if is_add then
user_id = v
end
end
...
if redis:sismember(RAMBOV2..hash, chat) then
user_id = Rm_Owner
end
if user_id then
local user_info = redis:get(RAMBOV2.."user:Name" .. user_id)
if user_info then
user = user_info
end
end
...
if hash then
local names = redis:hkeys(RAMBOV2..hash)
local text = ""
for i = 1, #names do
...
redis:set(RAMBOV2.."bot:Uptime", os.time())
----------------tdcli_update_callback---------------------------------------------------------------------------
function tdcli_update_callback(data)
local our_id = redis:get(RAMBOV2.."Our_ID") or 0
local api_id = redis:get(RAMBOV2.."Bot:Api_ID") or 0
if data.ID == "UpdateNewMessage" then
local msg = data.message_
local d = data.disable_notification_
local chat = chats[msg.chat_id_]
redis:sadd(RAMBOV2.."groups:users" .. msg.chat_id_, msg.sender_user_id_)--save users gp
redis:incr(RAMBOV2.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_.."")--save msgs gp
if msg.content_.ID == "MessageChatDeleteMember" then
if tonumber(msg.content_.user_.id_) == tonumber(_redis.Bot_ID) then
local user_info_ = redis:get(RAMBOV2.."user:Name" .. msg.sender_user_id_)
local UserRAMBOV2 = user_info_
if user_info_ then 
...
✓‏
‌‏]]
send(sudoed, 0, 1,iD_RAMBOV2, 1, "md")
redis:del(RAMBOV2.."bot:enable:" .. msg.chat_id_)
redis:srem(RAMBOV2.."bot:groups", msg.chat_id_)
end end end
if msg.content_.ID == "MessageChatAddMembers" then
redis:incr(RAMBOV2..'Rmaddcon'..msg.chat_id_..':'..msg.sender_user_id_)
if msg.date_ < os.time() - 40 then
print("\027[" .. color.white[1] .. ";" .. color.magenta[2] .. "m>>> OLD MSG <<<\027[00m")
return false
end
if not redis:get(RAMBOV2.."Set_Our_ID") then
...
if not redis:get(RAMBOV2.."Rank:Data") then
...
redis:set(RAMBOV2.."RmSudos" .. user, "> Sudo User ID : " .. sudouserid .. [[
...
redis:set(RAMBOV2.."RmOwnerBot" .. Rm_Owner, "> Bot Owner ID : " .. botowneruserid .. [[
...
redis:set(RAMBOV2.."Botid" .. result.id_, "> Bot ID : " .. botuserid .. [[
...
redis:set(RAMBOV2.."Botid" .. bot_id, [[
> Bot ID : ---
> Bot Name : ---
> Bot Username : ---
---------------]])
...
redis:setex(RAMBOV2.."Rank:Data", 700, true)
end
if redis:get(RAMBOV2.."bot:reload") and 30 > tonumber(redis:ttl(RAMBOV2.."bot:reload")) then
load_config()
setnumbergp()
redis:setex(RAMBOV2.."bot:reload", 7230, true)
print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m>>> Bot Reloaded <<<\027[00m")
end
if not redis:get(RAMBOV2.."bot:reload2") then
redis:del(RAMBOV2.."bot:groups")
redis:del(RAMBOV2.."bot:userss")
redis:setex(RAMBOV2.."bot:reloadingtime", 22 * hour, true)
redis:setex(RAMBOV2.."bot:reload2", week, true)
redis:setex(RAMBOV2.."bot:reload3", 2222 * day, true)
redis:setex(RAMBOV2.."bot:reload4", 2222 * day, true)
end
if redis:get(RAMBOV2.."bot:reload3") and 500 >= tonumber(redis:ttl(RAMBOV2.."bot:reload3")) then
local hash = "bot:groups"
local list = redis:smembers(RAMBOV2..hash)
for k, v in pairs(list) do
if not redis:get(RAMBOV2.."bot:enable:" .. v) and not redis:get(RAMBOV2.."bot:charge:" .. v) then
resetgroup(v)
chat_leave(v, bot_id)
redis:srem(RAMBOV2..hash, v)
end
end
redis:del(RAMBOV2.."bot:reload3")
end
if redis:get(RAMBOV2.."bot:reload4") and redis:ttl(RAMBOV2.."bot:reload4") <= 600 then
local reload_data_sudo = function()
local hashsudo = "Bot:RmSudos"
local listsudo = redis:smembers(RAMBOV2..hashsudo)
for k, v in pairs(listsudo) do
local hashdata = "sudo:data:" .. v
local listdata = redis:smembers(RAMBOV2..hashdata)
for k, gp in pairs(listdata) do
if not redis:sismember(RAMBOV2.."bot:groups", gp) then
redis:srem(RAMBOV2..hashdata, gp)
end
end
end
end
...
local listadmin = redis:smembers(RAMBOV2..hashadmin)
...
------------------------------EXpirepannel GP ----------------------------------------------------------
local expiretime = redis:ttl(RAMBOV2.."bot:charge:" .. msg.chat_id_)
if not redis:get(RAMBOV2.."bot:charge:" .. msg.chat_id_) and redis:get(RAMBOV2.."bot:enable:" .. msg.chat_id_) then
redis:del(RAMBOV2.."bot:enable:" .. msg.chat_id_)
redis:srem(RAMBOV2.."bot:groups", msg.chat_id_)
end
if redis:get(RAMBOV2.."bot:charge:" .. msg.chat_id_) and not redis:get(RAMBOV2.."bot:enable:" .. msg.chat_id_) then
redis:set(RAMBOV2.."bot:enable:" .. msg.chat_id_, true)
end
if not redis:get(RAMBOV2.."bot:expirepannel:" .. msg.chat_id_) and redis:get(RAMBOV2.."bot:charge:" .. msg.chat_id_) and tonumber(expiretime) < tonumber(day) and tonumber(expiretime) >= 3600 then
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") then
local v = tonumber(Rm_Owner)
local list = redis:smembers(RAMBOV2.."bot:owners:" .. msg.chat_id_)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(RAMBOV2.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
if user_info then
owner = user_info
else
...
send(v, 0, 1, "💲┊ سوف تنتهي صلاحيه المجموعه\n🔅┊الرابط : " .. (redis:get(RAMBOV2.."bot:group:link" .. msg.chat_id_) or "لا يوجد ") .. "\n🚫┊ الايدي » " .. msg.chat_id_ .. "", 1, "html")
redis:setex(RAMBOV2.."bot:expirepannel:" .. msg.chat_id_, 43200, true)
end
end
------------------------Autoleave FOR BOT----------------------------------------------------
if redis:get(RAMBOV2.."autoleave") == "On" then
local id = tostring(msg.chat_id_)
if not redis:get(RAMBOV2.."bot:enable:" .. msg.chat_id_) and id:match("-100(%d+)") and not redis:get(RAMBOV2.."bot:autoleave:" .. msg.chat_id_) then
redis:setex(RAMBOV2.."bot:autoleave:" .. msg.chat_id_, 1400, true)
end
local autoleavetime = tonumber(redis:ttl(RAMBOV2.."bot:autoleave:" .. msg.chat_id_))
local time = 400
if tonumber(autoleavetime) < tonumber(time) and tonumber(autoleavetime) > 150 then
redis:set(RAMBOV2.."lefting" .. msg.chat_id_, true)
end
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") and redis:get(RAMBOV2.."lefting" .. msg.chat_id_) then
if not redis:get(RAMBOV2.."bot:enable:" .. msg.chat_id_) and not redis:get(RAMBOV2.."bot:charge:" .. msg.chat_id_) then
redis:del(RAMBOV2.."lefting" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:autoleave:" .. msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
local v = tonumber(Rm_Owner)
send(v, 0, 1, "💲┊ تم مغادره المجموعــه\n🔱┊ الاسم » 👇🏾\n🏮┊ ("..title_name(msg.chat_id_)..")\n🚫┊ الايدي » " .. msg.chat_id_, 1, "html")
redis:srem(RAMBOV2.."bot:groups", msg.chat_id_)
elseif redis:get(RAMBOV2.."bot:enable:" .. msg.chat_id_) then
redis:del(RAMBOV2.."lefting" .. msg.chat_id_)
end
end
elseif redis:get(RAMBOV2.."bot:charge:" .. msg.chat_id_) == "Trial" and 500 > redis:ttl(RAMBOV2.."bot:charge:" .. msg.chat_id_) then
local v = tonumber(Rm_Owner)
send(v, 0, 1, "💲┊ تم مغادره المجموعــه\n🔱┊ الاسم » ("..title_name(msg.chat_id_)..")\n🚫┊ الايدي » " .. msg.chat_id_, 1, "html")
redis:srem(RAMBOV2.."bot:groups", msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
redis:del(RAMBOV2.."bot:charge:" .. msg.chat_id_)
end
...
redis:set(RAMBOV2..svgroup, chatname)
end
end
...
local id = result.id_
if username then
redis:set(RAMBOV2..svuser, "@" .. username)
else
redis:set(RAMBOV2..svuser, name)
end
end
getUser(msg.sender_user_id_, check_username)
if redis:get(RAMBOV2.."clerk") == "On" then
local clerk = function(extra, result)
if not is_admin(result.id_) then
local textc = redis:get(RAMBOV2.."textsec")
if not redis:get(RAMBOV2.."secretary_:" .. msg.chat_id_) and textc then
...
redis:setex(RAMBOV2.."secretary_:" .. msg.chat_id_, day, true)
end
end
end
...
local status_welcome = (redis:get(RAMBOV2..'status:welcome:'..msg.chat_id_) or 'disable')
...
if redis:get(RAMBOV2..'welcome:'..msg.chat_id_) then
text = redis:get(RAMBOV2..'welcome:'..msg.chat_id_)
else
...
----------------- save all msg bot --------------------------
redis:incr(RAMBOV2.."bot:allmsgs")
if msg.chat_id_ then
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") then
if not redis:sismember(RAMBOV2.."bot:groups", msg.chat_id_) then
redis:sadd(RAMBOV2.."bot:groups", msg.chat_id_)
end
elseif id:match("^(%d+)") then
if not redis:sismember(RAMBOV2.."bot:userss", msg.chat_id_) then
redis:sadd(RAMBOV2.."bot:userss", msg.chat_id_)
end
elseif not redis:sismember(RAMBOV2.."bot:groups", msg.chat_id_) then
redis:sadd(RAMBOV2.."bot:groups", msg.chat_id_)
end
end
---------------type the msg--------------------------------------------------
...
redis:incr(RAMBOV2.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")
text = msg.content_.text_
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Text ] <<\027[00m")
msg_type = "MSG:Text"
end
if msg.content_.ID == "MessagePhoto" then
redis:incr(RAMBOV2.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
redis:incr(RAMBOV2.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")
if not redis:get(RAMBOV2.."lock_STCK"..msg.chat_id_) then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"منور انت ، 😙","اه باع وجهك شكد نضيف 😅","هذا منو ، 😏","تسمحلي ابوسك ☹️😹","مليان ضحك مليان 😹❤️","تف على هذا  ويهك 💦😹","اذا حاته ممكن الرقم 😆😹","تدري صار "..(redis:get(RAMBOV2.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")).." ملصق داز  شهالتبذير 🤔😹","كافي ملصقات مشايف 😫"}
send(msg.chat_id_, msg.id_, 1,""..RAMBOV2[math.random(#RAMBOV2)].."", 1, 'md')
end
end
...
redis:incr(RAMBOV2.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Voice ] <<\027[00m")
msg_type = "MSG:Voice"
end
if msg.content_.ID == "MessageVideo" then
redis:incr(RAMBOV2.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Video ] <<\027[00m")
msg_type = "MSG:Video"
end
if msg.content_.ID == "MessageAnimation" then
redis:incr(RAMBOV2.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
redis:incr(RAMBOV2.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
local content_text = redis:get(RAMBOV2..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
if content_text == 'save_repgp' then
redis:del(RAMBOV2..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
local content_text = redis:get(RAMBOV2..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'')
if msg.content_.video_ then
redis:set(RAMBOV2..'video_repgp'..content_text..''..msg.chat_id_..'', msg.content_.video_.video_.persistent_id_)
end
if msg.content_.sticker_ then
redis:set(RAMBOV2..'stecker_repgp'..content_text..''..msg.chat_id_..'', msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
redis:set(RAMBOV2..'voice_repgp'..content_text..''..msg.chat_id_..'', msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.animation_ then
redis:set(RAMBOV2..'gif_repgp'..content_text..''..msg.chat_id_..'', msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
redis:set(RAMBOV2..'text_repgp'..content_text..''..msg.chat_id_..'', msg.content_.text_)
end
redis:sadd('rep_owner'..msg.chat_id_..'',content_text)
send(msg.chat_id_, msg.id_, 1, "📌┊ تم حفظ الرد بنجاح\n", 1, 'md')
redis:del(RAMBOV2..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'')
return false end end
if msg.content_.text_ and not redis:get(RAMBOV2..'lock_reeeep'..msg.chat_id_) then
if redis:get(RAMBOV2..'video_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(RAMBOV2..'video_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(RAMBOV2..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..'')  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(RAMBOV2..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if  redis:get(RAMBOV2..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(RAMBOV2..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(RAMBOV2..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(RAMBOV2..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(RAMBOV2..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
send(msg.chat_id_, msg.id_, 1, redis:get(RAMBOV2..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'') ,  1, 'md')
end
end
---------------------------------in all gps---------------------------------------------------
text = msg.content_.text_
if msg.content_.text_  or msg.content_.video_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ then
local content_text = redis:get(RAMBOV2.."add:repallt"..msg.sender_user_id_)
if content_text == 'save_rep' then
redis:del(RAMBOV2.."add:repallt"..msg.sender_user_id_)
local content_text = redis:get(RAMBOV2.."addreply2:"..msg.sender_user_id_)
if msg.content_.video_ then
redis:set(RAMBOV2.."video_repall"..content_text, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.sticker_ then
redis:set(RAMBOV2.."stecker_repall"..content_text, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
redis:set(RAMBOV2.."voice_repall"..content_text, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.animation_ then
redis:set(RAMBOV2.."gif_repall"..content_text, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
redis:set(RAMBOV2.."text_repall"..content_text, msg.content_.text_)
end
redis:sadd('rep_sudo',content_text)
send(msg.chat_id_, msg.id_, 1, "📌┊ تم حفظ الرد بنجاح\n", 1, 'md')
redis:del(RAMBOV2.."addreply2:"..msg.sender_user_id_)
return false end end
if msg.content_.text_ and not redis:get(RAMBOV2..'lock_reeeep'..msg.chat_id_) then
if redis:get(RAMBOV2.."video_repall"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(RAMBOV2.."video_repall"..msg.content_.text_))
end
if redis:get(RAMBOV2.."voice_repall"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(RAMBOV2.."voice_repall"..msg.content_.text_))
end
if  redis:get(RAMBOV2.."gif_repall"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(RAMBOV2.."gif_repall"..msg.content_.text_))
end
if redis:get(RAMBOV2.."stecker_repall"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(RAMBOV2.."stecker_repall"..msg.content_.text_))
end
if redis:get(RAMBOV2.."text_repall"..msg.content_.text_) then
send(msg.chat_id_, msg.id_, 1, redis:get(RAMBOV2.."text_repall"..msg.content_.text_) ,  1, "md")
end
end
...
if not redis:get(RAMBOV2..flmax) then
floodMax = 5
else
floodMax = tonumber(redis:get(RAMBOV2..flmax))
end
local pm = "flood:" .. msg.sender_user_id_ .. ":" .. msg.chat_id_ .. ":msgs"
if not redis:get(RAMBOV2..pm) then
msgs = 0
else
msgs = tonumber(redis:get(RAMBOV2..pm))
end
local TIME_CHECK = 2
local TIME_CHECK_PV = 2
local hashflood = "anti-flood:" .. msg.chat_id_
if msgs > floodMax - 1 then
if redis:get(RAMBOV2.."floodstatus" .. msg.chat_id_) == "Kicked" then
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
chat_kick(msg.chat_id_, msg.sender_user_id_)
elseif redis:get(RAMBOV2.."floodstatus" .. msg.chat_id_) == "DelMsg" then
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
else
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
end
...
if not redis:get(RAMBOV2..pmonpv) then
msgsonpv = 0
else
msgsonpv = tonumber(redis:get(RAMBOV2..pmonpv))
end
if msgsonpv > 12 then
blockUser(msg.sender_user_id_)
end
local idmem = tostring(msg.chat_id_)
if idmem:match("^(%d+)") and not is_admin(msg.sender_user_id_) and not redis:get(RAMBOV2.."Filtering:" .. msg.sender_user_id_) then
redis:setex(RAMBOV2..pmonpv, TIME_CHECK_PV, msgsonpv + 1)
end
...
if redis:get(RAMBOV2.."keed_link"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
local chat = msg.chat_id_
delete_msg(chat, msgs)
HTTPS.request("https://api.telegram.org/bot" .. RAMBOV2_TOKEN .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
redis:sadd(RAMBOV2..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
...
if redis:get(RAMBOV2.."keed_fosh"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."keed_user"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessagePhoto" then
if redis:get(RAMBOV2.."keed_photo" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."keed_text"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if text and text:match("(.*)(#)(.*)")  then
if redis:get(RAMBOV2.."keed_hashtag"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."keed_fwd"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageSticker" then
if redis:get(RAMBOV2.."keed_stecker"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageAudio" then
if redis:get(RAMBOV2.."keed_audio"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageVoice" then
if redis:get(RAMBOV2.."keed_voice"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageVideo" then
if redis:get(RAMBOV2.."keed_video"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageAnimation" then
if redis:get(RAMBOV2.."keed_gif"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageContact" then
if redis:get(RAMBOV2.."keed_contect"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."keed_arbic"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageDocument" then
if redis:get(RAMBOV2.."keed_Document"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."keed_english"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."keed_markdon"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not redis:get(RAMBOV2.."bot:muted:Time" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and redis:sismember(RAMBOV2.."bot:muted:" .. msg.chat_id_, msg.sender_user_id_) then
redis:srem(RAMBOV2.."bot:muted:" .. msg.chat_id_, msg.sender_user_id_)
end
if is_gbanned(msg.sender_user_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
return
end
if redis:get(RAMBOV2.."bot:muteall:Time" .. msg.chat_id_) then
local start_ = redis:get(RAMBOV2.."bot:muteall:start" .. msg.chat_id_)
local start = start_:gsub(":", "")
local stop_ = redis:get(RAMBOV2.."bot:muteall:stop" .. msg.chat_id_)
local stop = stop_:gsub(":", "")
local SVTime = os.date("%R")
local Time = SVTime:gsub(":", "")
if tonumber(Time) >= tonumber(start) and not redis:get(RAMBOV2.."bot:muteall:Run" .. msg.chat_id_) then
local g = os.time()
redis:set(RAMBOV2.."bot:muteall:start_Unix" .. msg.chat_id_, g)
...
redis:set(RAMBOV2.."bot:muteall:stop_Unix" .. msg.chat_id_, unix)
redis:set(RAMBOV2.."bot:muteall:Run" .. msg.chat_id_, true)
end
end
if redis:get(RAMBOV2.."bot:muteall:Time" .. msg.chat_id_) and redis:get(RAMBOV2.."bot:muteall:Run" .. msg.chat_id_) then
local SR = redis:get(RAMBOV2.."bot:muteall:start_Unix" .. msg.chat_id_) or 0
local SP = redis:get(RAMBOV2.."bot:muteall:stop_Unix" .. msg.chat_id_) or 0
...
redis:del(RAMBOV2.."bot:muteall:Run" .. msg.chat_id_)
end
end
---------------------pinnedmsg----------------------------------------------------------
if msg.content_.ID == "MessagePinMessage" and not msg.sender_user_id_ == our_id and not is_owner(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOV2.."pinnedmsg" .. msg.chat_id_) and redis:get(RAMBOV2.."bot:pin:mute" .. msg.chat_id_) then
unpinmsg(msg.chat_id_)
local pin_id = redis:get(RAMBOV2.."pinnedmsg" .. msg.chat_id_)
pinmsg(msg.chat_id_, pin_id, 0)
end
if not redis:get(RAMBOV2.."Resetdatapost" .. msg.chat_id_) then
redis:del(RAMBOV2.."Gp:Post" .. msg.chat_id_)
redis:setex(RAMBOV2.."Resetdatapost" .. msg.chat_id_, 12 * hour, true)
end
----------------------com viewget -----------------------------------------------------
if redis:get(RAMBOV2.."bot:viewget" .. msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, "✯↓ حدث خطا حاول مره اخرى", 1, "md")
redis:del(RAMBOV2.."bot:viewget" .. msg.sender_user_id_)
else
send(msg.chat_id_, msg.id_, 1, "📛↓ عـــدد المشاهـــدات \n: " .. msg.views_ .. "", 1, "md")
redis:del(RAMBOV2.."bot:viewget" .. msg.sender_user_id_)
end end
if redis:get(RAMBOV2.."bot:viewmsg") == "On" then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_) then
...
redis:del(RAMBOV2..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
setphoto(msg.chat_id_, photo_id)
end
------------------------------------------------------------------------------------------------
if redis:get(RAMBOV2.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local gps = redis:scard(RAMBOV2.."bot:groups") or 0
local gpss = redis:smembers(RAMBOV2.."bot:groups") or 0
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:del(RAMBOV2.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
if redis:get(RAMBOV2.."anti-flood:" .. msg.chat_id_) then
redis:setex(RAMBOV2..pm, TIME_CHECK, msgs + 1)
end
--«««««««««««««««««««««««« Developer By Karrar RAMBOV2 »»»»»»»»»»»»»»»»»»»»»»»»»»»--
if msg.content_.caption_ then
...
if redis:get(RAMBOV2.."bot:duplipost:mute" .. msg.chat_id_) then
if redis:sismember(RAMBOV2.."Gp:Post" .. msg.chat_id_, post) then
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:sadd(RAMBOV2.."Gp:Post" .. msg.chat_id_, post)
end
end
if redis:get(RAMBOV2.."post:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.forward_info_ and redis:get(RAMBOV2.."bot:forward:mute" .. msg.chat_id_) and (msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost") then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessagePhoto" then
if redis:get(RAMBOV2.."bot:photo:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:links:mute" .. msg.chat_id_) and (msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt].[Mm][Ee]")) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:strict" .. msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
end
end
if msg.content_.caption_:match("@") then
if redis:get(RAMBOV2.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityMention" and redis:get(RAMBOV2.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("#") then
if redis:get(RAMBOV2.."bot:hashtag:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityHashtag" and redis:get(RAMBOV2.."bot:hashtag:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and (msg.content_.entities_[0].ID == "MessageEntityTextUrl" or msg.content_.entities_[0].ID == "MessageEntityUrl") and redis:get(RAMBOV2.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.web_page_ and redis:get(RAMBOV2.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("[\216-\219][\128-\191]") and redis:get(RAMBOV2.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if (msg.content_.caption_:match("[A-Z]") or msg.content_.caption_:match("[a-z]")) and redis:get(RAMBOV2.."bot:english:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."markdown:lock" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."Game:lock" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOV2.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:document:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("[\216-\219][\121-\191]") and redis:get(RAMBOV2.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:inline:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."clerk") == "On" or is_admin(msg.sender_user_id_) then
...
if redis:get(RAMBOV2.."bot:sticker:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:tgservice:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOV2.."bot:member:lock" .. msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
end
...
for i = 0, #list do
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(RAMBOV2.."bot:bots:mute" .. msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
end
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(RAMBOV2.."bot:botskick" .. msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
chat_kick(msg.chat_id_, msg.sender_user_id_)
local user_info_ = redis:get(RAMBOV2..'user:Name' .. msg.sender_user_id_)
local UserRAMBOV2 = user_info_
if user_info_ then
send(msg.chat_id_, msg.id_, 1, "👨‍✈️┊ العضو » (["..UserRAMBOV2.."])\n🚫┊ الايدي » (*"..msg.sender_user_id_.."*)\n🗯┊ قام بأضافه بوت في المجموعه\n📌┊ تم طرد البوت مع العضو \n✓", 1, "md")
end 
end
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(RAMBOV2.."keed_bots"..msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
redis:sadd(RAMBOV2..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. RAMBOV2_TOKEN .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
redis:sadd(RAMBOV2..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
...
if redis:get(RAMBOV2.."bot:member:lock" .. msg.chat_id_) and not is_vipmem(msg.content_.members_[0].id_, msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
...
if redis:get(RAMBOV2.."bot:contact:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:music:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:voice:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:location:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:video:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:selfvideo:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:gifs:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_ then
check_filter_words(msg, msg.content_.caption_)
if redis:get(RAMBOV2.."bot:links:mute" .. msg.chat_id_) and (msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://")) then
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:setex(RAMBOV2.."bot:editid" .. msg.id_, day, msg.content_.text_)
if redis:get(RAMBOV2.."anti-flood:" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
redis:setex(RAMBOV2..pm, TIME_CHECK, msgs + 1)
end
---------------------rem cam filters---------------------------------------------------------------------
if redis:get(RAMBOV2.."Filtering:" .. msg.sender_user_id_) then
local chat = redis:get(RAMBOV2.."Filtering:" .. msg.sender_user_id_)
...
---------------save name bot-----------------------------
if redis:get(RAMBOV2..'botts:namess'..msg.sender_user_id_) then
redis:del(RAMBOV2..'botts:namess'..msg.sender_user_id_)
local NAME_BOT = msg.content_.text_:match("(.*)")
redis:set(RAMBOV2..'RAMBOV2nams',NAME_BOT)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم وضع اسم البوت 🍃",1, 'html')
return false
end
------------------------save cam link-----------------------------------
if redis:get(RAMBOV2.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and (msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")) then
local glink = msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")
local hash = "bot:group:link" .. msg.chat_id_
redis:set(RAMBOV2..hash, glink)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تـــم  حفــــظ الرابط 🎈", 1, "md")
redis:del(RAMBOV2.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
--------------------------------------------------------------------------
if redis:get(RAMBOV2.."gettextsec" .. msg.sender_user_id_) then
local clerktext = msg.content_.text_
redis:set(RAMBOV2.."textsec", clerktext)
send(msg.chat_id_, msg.id_, 1, "🌀┊  تم حفظ الكليشه 🎈", 1, "md")
redis:del(RAMBOV2.."gettextsec" .. msg.sender_user_id_)
end
---------------------save com rules-----------------------------------------------------------------------------
if redis:get(RAMBOV2.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local rules = msg.content_.text_
redis:set(RAMBOV2.."bot:rules" .. msg.chat_id_, rules)
send(msg.chat_id_, msg.id_, 1, "🌀┊  تـــم حفــــظ القوانين 🎋",  1, "md")
redis:del(RAMBOV2.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
-----------ch join------------------------------------------------------
if redis:get(RAMBOV2..'Rmch'..msg.sender_user_id_) then
redis:del(RAMBOV2..'Rmch'..msg.sender_user_id_)
local url , res = https.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/getChatAdministrators?chat_id='..msg.content_.text_..'')
local data = RmJS:decode(url)
if res == 400 then
...
redis:set(RAMBOV2..'Rmch1',CH_BOT)
...
--------------------rem  cam broadcast2--------------------------------------------------------------------------
if redis:get(RAMBOV2.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
if text:match("^الغاء$") then
send(msg.chat_id_, msg.id_, 1, "🔰- تم الغاء الامر بنجاح🎈 ", 1, "md")
redis:del(RAMBOV2.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
else
local gps = redis:scard(RAMBOV2.."bot:groups") or 0
local gpss = redis:smembers(RAMBOV2.."bot:groups") or 0
local msgs = {
...
if redis:get(RAMBOV2.."bot:joinbylink:" .. msg.sender_user_id_) and (msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")) then
else
end
--------------------set cam sudo  ------------------------------------------
if redis:get(RAMBOV2.."bot:RAMBOV2_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
redis:del(RAMBOV2.."bot:RAMBOV2_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
local RAMBOV2_dev = msg.content_.text_:match("(.*)")
redis:set(RAMBOV2.."RAMBOV2_dev", RAMBOV2_dev)
...
if (text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]")) and redis:get(RAMBOV2.."bot:links:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:text:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.text_:match("[\216-\219][\121-\191]") and redis:get(RAMBOV2.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not redis:get(RAMBOV2..hash) then
sens = 400
else
sens = tonumber(redis:get(RAMBOV2..hash))
end
if redis:get(RAMBOV2.."bot:spam:mute" .. msg.chat_id_) and string.len(msg.content_.text_) > sens or ctrl_chars > sens or real_digits > sens then
delete_msg(chat, msgs)
print_del_msg("Deleted Because [Lock] [Spam] ")
end
end
if (msg.content_.text_:match("[A-Z]") or msg.content_.text_:match("[a-z]")) and redis:get(RAMBOV2.."bot:english:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOV2.."bot:cmds" .. msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
...
-------------------------------leave groups----------------------------------------------------------------------
if is_sudo(msg) and idf:match("-100(%d+)") and (text:match('^'..(redis:get(RAMBOV2..'RAMBOV2nams') or 'كيبر')..' غادر$')) then
...
redis:set(RAMBOV2.."RAMBOV2_O" .. msg.chat_id_, uuuu[2])
end
local msgs = tonumber(redis:get(RAMBOV2.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local get_RAMBOV2 = tonumber(redis:get(RAMBOV2.."RAMBOV2_O" .. msg.chat_id_)) 
if msgs == get_RAMBOV2 and not redis:get(RAMBOV2.."lock_addd"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ هذا العضو اثبت تفاعله\n🎟┊تم رفعه ادمن في المجموعه\n✓",  1, "md")
redis:sadd(RAMBOV2..'bot:momod:'..msg.chat_id_, msg.sender_user_id_)
end
--------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "⚜️┊ تم حفظ العدد *"..uuuu[2].."*\n💬┊ سيتم رفع العضو مميز\n🚫┊اذا اثبت تفاعله\n✓",1, 'md')
redis:set(RAMBOV2.."RAMBOV2_OO" .. msg.chat_id_, uuuu[2])
end
local msgs = tonumber(redis:get(RAMBOV2.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local get_RAMBOV2 = tonumber(redis:get(RAMBOV2.."RAMBOV2_OO" .. msg.chat_id_)) 
if msgs == get_RAMBOV2 and not redis:get(RAMBOV2.."lock_adddvip"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "📌┊ هذا العضو اثبت تفاعله\n🌀┊تم رفعه عضو مميز في المجموعه\n✓",  1, "md")
redis:sadd(RAMBOV2..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_)
end
------------------------------ADD vipmems BY Reply------------------------------------------------------------------
if text:match('^رفع مميز عام$') and is_Rm(msg) and msg.reply_to_message_id_ ~= 0  then
function promote_by_reply(extra, result, success)
local user_info_ = redis:get(RAMBOV2..'user:Name' .. result.sender_user_id_)
local UserRAMBOV2 = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if redis:sismember(RAMBOV2..hash, result.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(RAMBOV2..hash, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم رفعه مميز عام\n✓', 1, 'md')
end end end
...
local hash = 'bot:vipmems:'
if result.id_ then
if redis:sismember(RAMBOV2..hash, result.id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(RAMBOV2..hash, result.id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفعه مميز عام\n✓‏', 1, 'md')
end end end
...
local user_info_ = redis:get(RAMBOV2..'user:Name' .. ap[2])
local UserRAMBOV2 = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if redis:sismember(RAMBOV2..hash, ap[2]) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(RAMBOV2..hash, ap[2])
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم رفعه مميز عام\n✓‏', 1, 'md')
end end end
...
if user_info_ then
local hash = 'bot:vipmems:'
if not redis:sismember(RAMBOV2..hash, result.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم تنزيله مميز عتم سابقا\n‏', 1, 'md')
else
redis:srem(RAMBOV2..hash, result.sender_user_id_)
...
local hash = 'bot:vipmems:'
if result.id_ then
if not redis:sismember(RAMBOV2..hash, result.id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تنزيله مميز عام سابقا\n‏', 1, 'md')
else
redis:srem(RAMBOV2..hash, result.id_)
...
if user_info_ then
local hash = 'bot:vipmems:'
if not redis:sismember(RAMBOV2..hash, ap[2]) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم تنزيله مميز عتم سابقا\n‏', 1, 'md')
else
redis:srem(RAMBOV2..hash, ap[2])
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم حذفه من مميزين العام \n✓‏', 1, 'md')
end end
end
-----------------------------promote_by_reply-------------------------------------------------------
...
function promote_by_username(extra, result, success)
if result.id_ then
if redis:sismember(RAMBOV2..'bot:vipmem:'..msg.chat_id_, result.id_) then
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ مرفوع مميز سابقا\n✓'
else
redis:sadd(RAMBOV2..'bot:vipmem:'..msg.chat_id_, result.id_)
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفعه عضو مميز\n✓'
end
...
local UserRAMBOV2 = user_info_
if user_info_ then
if redis:sismember(RAMBOV2..'bot:vipmem:'..msg.chat_id_, ap[2]) then
texts = '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ مرفوع مميز سابقا\n✓'
else
redis:sadd(RAMBOV2..'bot:vipmem:'..msg.chat_id_, ap[2])
texts = '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم رفعه عضو مميز\n✓'
end end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'')
redis:sadd(RAMBOV2..hash, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'')
redis:sadd(RAMBOV2..hash, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'')
redis:sadd(RAMBOV2..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOV2..hash, result.sender_user_id_)
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOV2..hash, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم الغاء تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOV2..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم الغاء تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
if result.id_ then
redis:sadd(RAMBOV2..'bot:admins:', result.id_)
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم اضافه ادمن في البوت \n✓‏'
else
...
local UserRAMBOV2 = user_info_
if user_info_ then
local k1 = not redis:sismember(RAMBOV2..'bot:owners:'..msg.chat_id_, result.sender_user_id_)
local k2 = not redis:sismember(RAMBOV2..'bot:momod:'..msg.chat_id_, result.sender_user_id_)
local k3 = not redis:sismember(RAMBOV2..'bot:vipmem:'..msg.chat_id_, result.sender_user_id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(RAMBOV2..'bot:owners:'..msg.chat_id_, result.sender_user_id_)
redis:srem(RAMBOV2..'bot:momod:'..msg.chat_id_, result.sender_user_id_)
redis:srem(RAMBOV2..'bot:vipmem:'..msg.chat_id_, result.sender_user_id_)
...
function delallrtb(extra, result, success)
if result.id_ then
local k1 = not redis:sismember(RAMBOV2..'bot:owners:'..msg.chat_id_, result.id_)
local k2 = not redis:sismember(RAMBOV2..'bot:momod:'..msg.chat_id_, result.id_)
local k3 = not redis:sismember(RAMBOV2..'bot:vipmem:'..msg.chat_id_, result.id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(RAMBOV2..'bot:owners:'..msg.chat_id_, result.id_)
redis:srem(RAMBOV2..'bot:momod:'..msg.chat_id_, result.id_)
redis:srem(RAMBOV2..'bot:vipmem:'..msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم حذف كل الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
local UserRAMBOV2 = user_info_
if user_info_ then
local k1 = not redis:sismember(RAMBOV2..'bot:owners:'..msg.chat_id_, ap[2])
local k2 = not redis:sismember(RAMBOV2..'bot:momod:'..msg.chat_id_, ap[2])
local k3 = not redis:sismember(RAMBOV2..'bot:vipmem:'..msg.chat_id_, ap[2])
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(RAMBOV2..'bot:owners:'..msg.chat_id_, ap[2])
redis:srem(RAMBOV2..'bot:momod:'..msg.chat_id_, ap[2])
redis:srem(RAMBOV2..'bot:vipmem:'..msg.chat_id_, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم حذف كل الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
local UserRAMBOV2 = user_info_
if user_info_ then
local k1 = not redis:sismember(RAMBOV2..'bot:keed:'..msg.chat_id_, result.sender_user_id_)
local k2 = not redis:sismember(RAMBOV2..'bot:muted:'..msg.chat_id_, result.sender_user_id_)
local k3 = not redis:sismember(RAMBOV2..'bot:banned:'..msg.chat_id_, result.sender_user_id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ لا توجد لديه قيـــــــود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOV2..'bot:keed:'..msg.chat_id_, result.sender_user_id_)
redis:srem(RAMBOV2..'bot:muted:'..msg.chat_id_, result.sender_user_id_)
redis:srem(RAMBOV2..'bot:banned:'..msg.chat_id_, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
function delallrtb(extra, result, success)
if result.id_ then
local k1 = not redis:sismember(RAMBOV2..'bot:keed:'..msg.chat_id_, result.id_)
local k2 = not redis:sismember(RAMBOV2..'bot:muted:'..msg.chat_id_, result.id_)
local k3 = not redis:sismember(RAMBOV2..'bot:banned:'..msg.chat_id_, result.id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ لا توجد لديه قيـــــــود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOV2..'bot:keed:'..msg.chat_id_, result.id_)
redis:srem(RAMBOV2..'bot:muted:'..msg.chat_id_, result.id_)
redis:srem(RAMBOV2..'bot:banned:'..msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
local UserRAMBOV2 = user_info_
if user_info_ then
local k1 = not redis:sismember(RAMBOV2..'bot:keed:'..msg.chat_id_, ap[2])
local k2 = not redis:sismember(RAMBOV2..'bot:muted:'..msg.chat_id_, ap[2])
local k3 = not redis:sismember(RAMBOV2..'bot:banned:'..msg.chat_id_, ap[2])
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ لا توجد لديه قيـــــــود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOV2..'bot:keed:'..msg.chat_id_, ap[2])
redis:srem(RAMBOV2..'bot:muted:'..msg.chat_id_, ap[2])
redis:srem(RAMBOV2..'bot:banned:'..msg.chat_id_, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOV2..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
if redis:get(RAMBOV2.."lock_GEM"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ اللعبه معطله\n ‏ ", 1, "md")
return false end
local user_info_ = redis:get(RAMBOV2.."user:Name" .. msg.sender_user_id_)
local UserRAMBOV2 = user_info_
if user_info_ then
redis:set(RAMBOV2..'kk1'..msg.sender_user_id_..''..msg.chat_id_..'','kk')
send(msg.chat_id_, 0, 1, '👨‍✈️» اهلا ['..UserRAMBOV2..'] \n™️» في لعبه السرعه\n⚠️» ارسل ( بدء اللعبه ) للعب\n✓',1, 'md')
return false end end
if text == 'بدء اللعبه'  and is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOV2..'kk1'..msg.sender_user_id_..''..msg.chat_id_..'') then
local keep1 = {'س م ى و','ى-س-م-و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep1[math.random(#keep1)]..')*',1, 'md')
redis:set(RAMBOV2..'kk11'..msg.chat_id_..'','kkk')
return false end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'kk11'..msg.chat_id_..'')
if RAMBOV21 == 'kkk' then
if text == 'موسى' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'kk11'..msg.chat_id_..'')
sleep(1.5)
local ooo = {'ا-ل-ي-ي-ب','ي ي ا ل ب'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ooo[math.random(#ooo)]..')*',1, 'md')
redis:set(RAMBOV2..'kk111'..msg.chat_id_..'','kkkk')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'kk111'..msg.chat_id_..'')
if RAMBOV21 == 'kkkk' then
if text == 'ليبيا' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'kk111'..msg.chat_id_..'')
sleep(1.5) 
local keep = {'ر ط ا ي ه','ا-ي-ط-ر-ه'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep[math.random(#keep)]..')*',1, 'md')
redis:set(RAMBOV2..'kk1111'..msg.chat_id_..'','mm')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'kk1111'..msg.chat_id_..'')
if RAMBOV21 == 'mm' then
if text == 'طياره' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'kk1111'..msg.chat_id_..'')
sleep(1.5)
local keep77 = {'😛😛😛😝😛😛😛','😜😝😜😜😜😜😜'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep77[math.random(#keep77)]..')*',1, 'md')
redis:set(RAMBOV2..'w1'..msg.chat_id_..'','q1')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'w1'..msg.chat_id_..'')
if RAMBOV21 == 'q1' then
if text == '😝' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'w1'..msg.chat_id_..'')
sleep(1.5)
local keep2 = {'ش-ر-ط-ي','ش,ط,ر,ي'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep2[math.random(#keep2)]..')*',1, 'md')
redis:set(RAMBOV2..'w2'..msg.chat_id_..'','q2')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'w2'..msg.chat_id_..'')
if RAMBOV21 == 'q2' then
if text == 'شرطي' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'w2'..msg.chat_id_..'')
sleep(1.5)
local keep3 = {'😃😃😄😃😃😃'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep3[math.random(#keep3)]..')*',1, 'md')
redis:set(RAMBOV2..'w9'..msg.chat_id_..'','q9')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'w9'..msg.chat_id_..'')
if RAMBOV21 == 'q9' then
if text == '😄' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'w9'..msg.chat_id_..'')
sleep(1.5)
local keep4 = {'ر ك و ا ي','ر ا ي ك و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep4[math.random(#keep4)]..')*',1, 'md')
redis:set(RAMBOV2..'w4'..msg.chat_id_..'','q4')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'w4'..msg.chat_id_..'')
if RAMBOV21 == 'q4' then
if text == 'كوريا' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'w4'..msg.chat_id_..'')
sleep(1.5)
local keep5 = {'ك - م - ا - ل','ل ك ا م'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep5[math.random(#keep5)]..')*',1, 'md')
redis:set(RAMBOV2..'w5'..msg.chat_id_..'','q5')
return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'w5'..msg.chat_id_..'')
if RAMBOV21 == 'q5' then
if text == 'ملاك' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'w5'..msg.chat_id_..'')
sleep(1.5)
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n(ك , ش , ب , ا)',1, 'md')
redis:set(RAMBOV2..'w6'..msg.chat_id_..'','q6')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'w6'..msg.chat_id_..'')
if RAMBOV21 == 'q6' then
if text == 'شباك' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'w6'..msg.chat_id_..'')
sleep(1.5)
local keep6 = {'ل-ج-م-ي','ل ي م ج'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep6[math.random(#keep6)]..')*',1, 'md')
redis:set(RAMBOV2..'w7'..msg.chat_id_..'','q7')
return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'w7'..msg.chat_id_..'')
if RAMBOV21 == 'q7' then
if text == 'جميل' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'w7'..msg.chat_id_..'')
sleep(1.5)
local keep7 = {'و ك ه ه','ه ك ه و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep7[math.random(#keep7)]..')*',1, 'md')
redis:set(RAMBOV2..'w8'..msg.chat_id_..'','q8')
return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'w8'..msg.chat_id_..'')
if RAMBOV21 == 'q8' then
if text == 'كهوه' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'w8'..msg.chat_id_..'')
sleep(1.5)
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n( ن ،ص، و، ك، د)',1, 'md')
redis:set(RAMBOV2..'w9o'..msg.chat_id_..'','q9o')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'w9o'..msg.chat_id_..'')
if RAMBOV21 == 'q9o' then
if text == 'صندوك' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'w9o'..msg.chat_id_..'')
sleep(1.5)
local keep8 = {'ض ر م ي','م ض ر ي'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep8[math.random(#keep8)]..')*',1, 'md')
redis:set(RAMBOV2..'w00'..msg.chat_id_..'','q00')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'w00'..msg.chat_id_..'')
if RAMBOV21 == 'q00' then
if text == 'مريض' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'w00'..msg.chat_id_..'')
sleep(1.5)
local keep9 = {'ي- ط -ر -ق -ن -ب -ا',' ب ي ق ا ط ر ا ن','ب,ي,ا,,ق,ر,ن,ط'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep9[math.random(#keep9)]..')*',1, 'md')
redis:set(RAMBOV2..'a15'..msg.chat_id_..'','s15')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'a15'..msg.chat_id_..'')
if RAMBOV21 == 's15' then
if text == 'قرنابيط' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'a15'..msg.chat_id_..'')
sleep(1.5)
local keep00 = {'😔😔😔😔😔😔😔☺️😔😔😔😔😔'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep00[math.random(#keep00)]..')*',1, 'md')
redis:set(RAMBOV2..'a26'..msg.chat_id_..'','s26')
return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'a26'..msg.chat_id_..'')
if RAMBOV21 == 's26' then
if text == '☺️' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'a26'..msg.chat_id_..'')
sleep(1.5)
local keepp = {'ط - م - ي','ي ,م ,ط'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keepp[math.random(#keepp)]..')*',1, 'md')
redis:set(RAMBOV2..'a99'..msg.chat_id_..'','s99')
 return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'a99'..msg.chat_id_..'')
if RAMBOV21 == 's99' then
if text == 'مطي' then
redis:incr(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOV2..'a99'..msg.chat_id_..'')
return false end end end
-----------------------------------------------------------------------------
if text == 'نقاطي' then
send(msg.chat_id_, msg.id_, 1, '💬┊ عدد نقاطك » *('..tonumber(redis:get(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )..')*',1, 'md')
return false end
if text == 'بيع نقاطي' then
local user_info_ = redis:get(RAMBOV2..'user:Name' .. msg.sender_user_id_)
local UserRAMBOV2 = user_info_
if user_info_ then
local numkep = tonumber(redis:get(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
if numkep == 0 then
...
redis:set(RAMBOV2..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'','karrar2')
send(msg.chat_id_, msg.id_, 1, keep,1, 'md')
return false end end end
if text then
local RAMBOV21 = redis:get(RAMBOV2..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
if RAMBOV21 == 'karrar2' then
if text == '1' then
if redis:sismember(RAMBOV2..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'⚠️┊ عذرا انت مرفوع مميز سابقا\n✓', 1, 'md')
redis:del(RAMBOV2..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
else
local numkep = tonumber(redis:get(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
if numkep < tonumber(redis:get(RAMBOV2.."RAMBOV2_OO0" .. msg.chat_id_) or 100 )  then
send(msg.chat_id_, msg.id_, 1, '®️┊عذرا نقاطك اقل من '..tonumber(redis:get(RAMBOV2.."RAMBOV2_OO0" .. msg.chat_id_) or 100 )..'\n💬┊لا تستطيع تحويلهم لعضو مميز\n✓',1, 'md')
redis:del(RAMBOV2..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
return false end
if numkep > tonumber(redis:get(RAMBOV2.."RAMBOV2_OO0" .. msg.chat_id_) or 100 )  then
send(msg.chat_id_, msg.id_, 1, '🎰┊مبروك عزيزي\n🎧┊تم رفعك عضو مميز\n ✓',1, 'md')
redis:sadd(RAMBOV2..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_)
redis:del(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(RAMBOV2..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تفعيل امر اطردني\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."lock_kickme"..msg.chat_id_, true)
return false 
end
if text == 'تعطيل اطردني' and is_owner(msg.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تعطيل امر اطردني\n ✓ ", 1, 'md')
redis:del(RAMBOV2.."lock_kickme"..msg.chat_id_)
return false 
end
-------------------add ch --------------------------
...
redis:setex(RAMBOV2..'Rmch'..msg.sender_user_id_,300,true)
...
if not redis:get(RAMBOV2..'Rmch1') then
send(msg.chat_id_, msg.id_, 1, '™️┊ لم يتم وضع القناة\n📌┊ لتعين  القناة ارسل .....\n⚠️┊ `تعين قناة الاشتراك `\n➖', 1, 'md')
return false 
end
if redis:get(RAMBOV2..'Rmch1') then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تفعيل الاشتراك الاجباري\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."Rmjoin1", true)
return false end end end
...
redis:del(RAMBOV2.."dellmssg"..msg.chat_id_)
end
----------------ADD DEL MSG------------------------------------------------------------
...
redis:set(RAMBOV2.."dellmssg"..msg.chat_id_, true)
end
----------add - rem bot free----------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تفعيل البوت خدمي\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."bot:free", true)
end end
...
local UserRAMBOV2 = user_info_
if user_info_ then
local start = redis:get(RAMBOV2.."startbot")
if start then
keep = [[
[]]..start..[[]
]]
...
💠┊مرحبا انا بوت اسمي *(]]..(redis:get(RAMBOV2..'RAMBOV2nams') or 'كيبر')..[[)🗼*
...
🔱┊*مطور البوت* » ( []] .. UserRAMBOV2 .. [[] )‏
‏
‏]]
send(msg.chat_id_, msg.id_, 1, RAMBOV2, 1, 'md')
end end end
-----------------------get start------------------------------------------------------------------------
...
local Rmch2 = redis:get(RAMBOV2.."Rmch1")
if Rmch2 then
...
text = "🌀┊  ادمنيه البوت :\n*≖≖≖≖≖≖≖≖≖≖≖≖*\n"
for k, v in pairs(list) do
if redis:get(RAMBOV2.."SudoNumberGp" .. v) then
gps = tonumber(redis:get(RAMBOV2.."SudoNumberGp" .. v))
else
gps = 0
end
local user_info = redis:get(RAMBOV2.."user:Name" .. v)
...
local time = a[2] * day
redis:setex(RAMBOV2.."bot:charge:"..msg.chat_id_,time,true)
redis:set(RAMBOV2.."bot:enable:"..msg.chat_id_,true)
end
------------------charge-----------------------------------------------------------------------------
if text:match("^فحص الشحن") and is_momod(msg.sender_user_id_, msg.chat_id_) then
local ex = redis:ttl(RAMBOV2.."bot:charge:"..msg.chat_id_)
if ex == -1 then
...
local ex = redis:ttl(RAMBOV2.."bot:charge:"..txt[2])
if ex == -1 then
...
if not redis:get(RAMBOV2.."bot:free") and not is_sudo(msg) then
send(msg.chat_id_, msg.id_, 1, '🚫┊* للمطوريـــن فقـــط* ❗️', 1, 'md')
return false end
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(RAMBOV2..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
if result.members_[i].status_.ID == "ChatMemberStatusCreator" then
owner_id = admins[i].user_id_
redis:sadd(RAMBOV2.."bot:owners:"..msg.chat_id_,owner_id)
end
end
end
getChannelMembers(msg.chat_id_, 0, 'Administrators', 200, promote_admin)
redis:set(RAMBOV2.."test:group"..msg.chat_id_,'krar')
if redis:get(RAMBOV2.."bot:enable:" .. msg.chat_id_) then
...
if not redis:get(RAMBOV2.."bot:group:link"..msg.chat_id_) then
local getlink = 'https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = RmJS:decode(req)
if link.ok == true then 
redis:set(RAMBOV2.."bot:group:link"..msg.chat_id_,link.result)
end
end
local lik_1 = redis:get(RAMBOV2.."bot:group:link"..msg.chat_id_)
if lik_1 then
link = lik_1 
else
link = link.result
end
send(Rm_Owner,0, 1,"- *تم اضافه مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ المدير ≈ [" .. owner .. "]\n🔰┊ ["..title_name(msg.chat_id_).."]("..(link or "t.me/RAMBOV2_ch")..")\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " ..result.first_name_.. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n✓", 1, "md")
redis:set(RAMBOV2.."bot:enable:" .. msg.chat_id_, true)
redis:setex(RAMBOV2.."bot:charge:" .. msg.chat_id_, 9999 * day, true)
redis:sadd(RAMBOV2.."sudo:data:" .. msg.sender_user_id_, msg.chat_id_)
end
tdcli_function ({
ID = "GetChannelFull",
...
redis:set(RAMBOV2.."bot:charge:"..txt[2],true)
...
local txt = { string.match(text, "^(تفعيل)$")}
redis:del(RAMBOV2..'lock:add'..msg.chat_id_)
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(RAMBOV2..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
...
redis:set(RAMBOV2..'lock:add'..msg.chat_id_,true)
if not redis:get(RAMBOV2.."bot:enable:" .. msg.chat_id_) then
...
redis:del(RAMBOV2.."bot:enable:" .. msg.chat_id_)
redis:srem(RAMBOV2.."bot:groups", msg.chat_id_)
redis:del(RAMBOV2.."test:group"..msg.chat_id_)
redis:srem(RAMBOV2.."sudo:data:" .. msg.sender_user_id_, msg.chat_id_)
...
send(v,0, 1,"- *تم تعطيل مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ المدير ≈ " .. owner .. "\n🔰┊ الرابط ≈ [اضغـط هنـا](" .. (redis:get(RAMBOV2.."bot:group:link"..msg.chat_id_) or "https://t.me/RAMBOV2_ch") .. ")\n🔅┊* اسم المجموعه* »\n👨🏼┊ﮧ "..title_name(msg.chat_id_).."\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " .. fname .. " " .. lname .. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n‏", 1, "md")
end
...
local gp = txt[3]
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم تفعيل المجموعه \n(*"..txt[2].."*) يوم 🍃", 1, 'html')
redis:sadd(RAMBOV2..'sudo:data:'..sudo, gp)
end
-------------------REEM----------------------------------------------------------------------------
...
local send_to_bot_owner = function(extra, result)
redis:del(RAMBOV2.."bot:enable:" .. gp[2])
redis:del(RAMBOV2.."bot:charge:" .. gp[2])
local v = tonumber(Rm_Owner)
local fname = result.first_name_ or ""
local lname = result.last_name_ or ""
local username = "@" .. result.username_ or ""
...
redis:srem(RAMBOV2.."sudo:data:" .. msg.sender_user_id_, gp[2])
redis:srem(RAMBOV2.."bot:groups", gp[2])
end
getUser(msg.sender_user_id_, send_to_bot_owner)
end
else
...
local num_keep = (tonumber(redis:get(RAMBOV2.."incr_msg"..result.id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(RAMBOV2.."msgs:"..result.id_..":"..msg.chat_id_) or  0))
local Rmcontact = (tonumber(redis:get(RAMBOV2.."Rmaddcon"..msg.chat_id_..":"..result.id_) or 0))
if result.id_ then
...
local UserRAMBOV2 = user_info_
if user_info_ then
send(msg.chat_id_, msg.id_, 1, "•اهلا ~ ["..UserRAMBOV2.."]\n🎧» ايديك  (`"..msg.sender_user_id_.."`)\n✓", 1, "md")
return false end end
-------------------RETBA-------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local num_keep = tonumber(redis:get(RAMBOV2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
local msgs = tonumber(redis:get(RAMBOV2.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local Rmcontact = (tonumber(redis:get(RAMBOV2.."Rmaddcon"..msg.chat_id_..":"..msg.sender_user_id_) or 0))
...
if redis:get(RAMBOV2.."getidstatus" .. msg.chat_id_) == "Photo" then
...
if redis:get(RAMBOV2.."getidstatus" .. msg.chat_id_) == "Simple" then
send(msg.chat_id_, msg.id_, 1, kepper_info2, 1, "md")
end
if not redis:get(RAMBOV2.."getidstatus" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, kepper_info2, 1, "md")
 end 
 end
tdcli_function({
...
--------------------Rm concat add----------------------------------------
if text == 'جهاتي' then
send(msg.chat_id_, msg.id_, 1, '🔰┊ عدد جهاتك ≈ * '..(tonumber(redis:get(RAMBOV2..'Rmaddcon'..msg.chat_id_..':'..msg.sender_user_id_) or 0))..' *',1,'md')
end
--------------------ID BY REPLY------------------------------------------
if text:match("^ايدي$") and msg.reply_to_message_id_ ~= 0 then
function iD_reP(extra, result, success)
local num_keep = (tonumber(redis:get(RAMBOV2.."incr_msg"..result.sender_user_id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(RAMBOV2.."msgs:"..result.sender_user_id_..":"..msg.chat_id_) or 0 ))
local Rmcontact = (tonumber(redis:get(RAMBOV2.."Rmaddcon"..msg.chat_id_..":"..result.sender_user_id_) or 0))
if result.id_ then
if tonumber(result.sender_user_id_) == tonumber(Rm_Owner) then
RAMBOV23 = "مطور الاساسـي 🌿"
elseif is_sudoid(result.sender_user_id_) then
RAMBOV23 = "المطور 🐾"
elseif is_admin(result.sender_user_id_) then
RAMBOV23 = "ادمن في البوت"
elseif is_vipmems(result.sender_user_id_) then
RAMBOV23 = "مميز عام 🌿"
elseif is_monshi(result.sender_user_id_, msg.chat_id_) then
RAMBOV23 = "منشىء الكروب 🐾"
elseif is_owner(result.sender_user_id_, msg.chat_id_) then
RAMBOV23 = "المدير 🌿"
elseif is_momod(result.sender_user_id_, msg.chat_id_) then
RAMBOV23 = "ادمن المجموعه🐾"
...
local RAMBOV2_info = "🎟┊ ايديه  » `" .. result.sender_user_id_ .. "`\n👤┊ جهاته  »   "..Rmcontact.."\n🔆┊ نقاطه  »   "..num_keep.."\n🌐┊ تفاعله »  " .. Rm_TM_NM(msgs) .. "\n✉️┊ رسائله » " .. msgs .. "\n📌┊ موقعه » " .. RAMBOV23 .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
send(msg.chat_id_, result.id_, 1, RAMBOV2_info, 1, "md")
end
...
redis:setex(RAMBOV2..'botts:namess'..msg.sender_user_id_,698,true)
...
if redis:get(RAMBOV2.."getpro:" .. msg.chat_id_) == "Active" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تفعيل جلب الصور\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تفعيل جلب الصور\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."getpro:" .. msg.chat_id_, "Active")
end end end
...
if redis:get(RAMBOV2.."getpro:" .. msg.chat_id_) == "Deactive" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تعطيل جلب الصور\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تعطيل جلب الصور\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."getpro:" .. msg.chat_id_, "Deactive")
end end end
---------------------------------------------------------------------
if text:match("^مسح الصوره") and is_momod(msg.sender_user_id_, msg.chat_id_)  then
https.request('https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/deleteChatPhoto?chat_id='..msg.chat_id_)
...
if not is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOV2.."getpro:" .. msg.chat_id_) == "Deactive" then
...
redis:set(RAMBOV2..'flood:time:'..msg.chat_id_,floodt[2])
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم وضع التكرار بالطرد\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."floodstatus" .. msg.chat_id_, "Kicked")
end
end
if status[2] == "بالمسح" then
if redis:get(RAMBOV2.."floodstatus" .. msg.chat_id_) == "DelMsg" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم وضع التكرار بالمسح\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم وضع التكرار بالمسح\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."floodstatus" .. msg.chat_id_, "DelMsg")
end
end
end
-------------------SET warn-----------------------------------------
...
redis:set(RAMBOV2.."warn:max:" .. msg.chat_id_, warnmax[2])
end
end
---------------------warnstatus----------------------------------------------------------
...
if redis:get(RAMBOV2.."warnstatus" .. msg.chat_id_) == "Muteuser" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم وضع التحذير بالكتم\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم وضع التحذير بالكتم\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."warnstatus" .. msg.chat_id_, "Muteuser")
end   end
if status[2] == "بالطرد" then
if redis:get(RAMBOV2.."warnstatus" .. msg.chat_id_) == "Remove" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم وضع التحذير بالطرد\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم وضع التحذير بالطرد\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."warnstatus" .. msg.chat_id_, "Remove")
end end end
-----------------------getidstatus-----------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تفعيل الايدي بالصوره\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."getidstatus" .. msg.chat_id_, "Photo")
end end  end
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تعطيل الايدي بالصوره\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."getidstatus" .. msg.chat_id_, "Simple")
end end end
-------------------autoleave-----------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تفعيل الخروج التلقائي\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."autoleave", "On")
end end end
if is_sudo(msg) and text:match("^تعطيل (.*)$") then
local status = { string.match(text, "^(تعطيل) (.*)$")}
if status[2] == "الخروج التلقائي" then
if redis:get(RAMBOV2.."autoleave") == "Off" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تعطيل الخروج التلقائي\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تعطيل الخروج التلقائي\n ✓ ", 1, 'md')
redis:set(RAMBOV2.."autoleave", "Off")
end   end  end
-----------------------------------------------------------
...
redis:setex(RAMBOV2.."bot:RAMBOV2_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 370, true)
end end
...
local RAMBOV2_dev = redis:get(RAMBOV2.."RAMBOV2_dev")
if RAMBOV2_dev then
send(msg.chat_id_, msg.id_, 1, "["..RAMBOV2_dev.."]", 1, "md")
else
local karrar = [[
...
redis:setex(RAMBOV2.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 1200, true)
end
if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^حذف الرابط$"))  then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم حذف الرابط بنجاح\n ✓ ", 1, 'md')
redis:del(RAMBOV2.."bot:group:link" .. msg.chat_id_)
end
if text:match("^الرابط$") then
local link = redis:get(RAMBOV2.."bot:group:link" .. msg.chat_id_)
if link then
...
redis:setex(RAMBOV2.."Filtering:" .. msg.sender_user_id_, 80, msg.chat_id_)
end
----------------------------------------------------------------------------------
...
local hash = "bot:filters:" .. msg.chat_id_
if not redis:hget(RAMBOV2..hash, name) then
send(msg.chat_id_, msg.id_, 1, "🌀┊  الكلمه〖  " .. name .. "  〗\n👥┊  تم الغاء منعها📍", 1, "md")
redis:hdel(RAMBOV2..hash, name)
end end
-------------------USERS----------------------------------------------------
if text == 'المشتركين' or text == 'عدد المشتركين' and is_Rm(msg) then
local users = redis:scard(RAMBOV2.."bot:userss")
local botnamess = redis:get(RAMBOV2.."RAMBOV2nams") or "كيبر"
...
local get_me = function(extra, result)
local num = (redis:scard(RAMBOV2.."bot:groups"))
local list = redis:smembers(RAMBOV2.."bot:groups")
local text = "~ All_Groups_Bots \n\n"
for k,v in pairs(list) do
local GroupsOwner = redis:scard(RAMBOV2.."bot:owners:"..v) or 0
local GroupsMod = redis:scard(RAMBOV2.."bot:momod:"..v) or 0
local Groupslink = redis:get(RAMBOV2.."bot:group:link" ..v)
if result.first_name_ then
if #result.first_name_ < 35 then
else
for kkkkk in string.gmatch(result.first_name_, "[^%s]+") do
...
file:write(text)
file:close()
local token_files = 'https://api.telegram.org/bot' .. RAMBOV2_TOKEN .. '/sendDocument'
...
local botnamess = redis:get(RAMBOV2.."RAMBOV2nams") or "كيبر"
local RmSudos1 = redis:scard(RAMBOV2.."Bot:RmSudos") or "0"
local RmAdmins = redis:scard(RAMBOV2.."Bot:Admins") or "0"
local Rmgban = redis:scard(RAMBOV2.."bot:gban:") or "0"
local Rmvipmems = redis:scard(RAMBOV2.."bot:vipmems:") or "0"
local Rmgps = redis:scard(RAMBOV2.."bot:groups") or "0"
local Rmusers = redis:scard(RAMBOV2.."bot:userss") or "0"
send(msg.chat_id_, msg.id_, 1, "*- احصائيات البـــــوت ≈*\n*ﮧ -------------------»*\n👨🏼┊اسم البوت ≈ *" .. botnamess .. "*\n🃏┊عدد المطورين ≈ *"..RmSudos1.."*\n🔰┊عدد الادمنيه ≈ *"..RmAdmins.."*\n🔚┊عدد الكروبات ≈ * "..Rmgps.."*\n👪┊عدد المشتركين خاص ≈ *" .. Rmusers .. "*\n🔔┊عدد المحظورين عام ≈ *"..Rmgban.."*\n🔘┊عدد المميزين عام ≈ * "..Rmvipmems.."*\n🗯┊قناة السورس ≈ [RAMBOV2](t.me/RAMBOV2_ch)\n✓", 1,"md")
end
end
------------------GPS AND USERS-------------------------------------------
if text:match("^المجموعات$") or text:match("^الكروبات$") and is_admin(msg.sender_user_id_, msg.chat_id_) then
local gps = redis:scard(RAMBOV2.."bot:groups")
local users = redis:scard(RAMBOV2.."bot:userss")
...
function cb(a,b,c)
local text = b.content_.text_
local users = redis:scard(RAMBOV2.."bot:userss") or 0
local userss = redis:smembers(RAMBOV2.."bot:userss") or 0
local msgs = {
[0] = id
}
for i = 1, #userss do
...
if redis:get(RAMBOV2.."ffosh"..msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = { [0] = id}
local chat = msg.chat_id_
delete_msg(chat, msgs)
...
send(msg.chat_id_, msg.id_, 1, "🚫┊ الرتبه » (*"..RAMBOV23.."*) 🍃\n‏" , 1, "md")
end
...
`git clone https://github.com/ramizob3a/RAMBOV2.git && cd RAMBOV2 && chmod +x RAMBOV2 && ./RAMBOV2`
...
`killall screen && cd RAMBOV2 && screen ./RAMBOV2`

💬┊ *مطور السورس *≈ [@r](t.me/r)
💲┊ *قناٌة السوٰرس *≈ [@k](T.ME/k)
...
redis:set(RAMBOV2..'welcome:'..msg.chat_id_,welcome)
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم وضع الترحيب\n ✓ ", 1, 'md')
end
-----------------welcome------------------------------------------
if text:match("^الترحيب$") and is_momod(msg.sender_user_id_, msg.chat_id_) then
local wel = redis:get(RAMBOV2..'welcome:'..msg.chat_id_)
if wel then
...
redis:setex(RAMBOV2.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
...
redis:setex(RAMBOV2.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
send(msg.chat_id_, msg.id_, 1, "🌀┊  ارسل لي الرساله الان📲\n🚫┊ للخروج ارسل لي الغاء 📍 ", 1, "md")
end end end
----------------- info bot --------------------------------------------------------------
...
local gps = redis:scard(RAMBOV2.."bot:groups")
local users = redis:scard(RAMBOV2.."bot:userss")
local allmgs = redis:get(RAMBOV2.."bot:allmsgs")
if redis:get(RAMBOV2.."autoleave") == "On" then
autoleaveAR = "مفعل"
elseif redis:get(RAMBOV2.."autoleave") == "Off" then
autoleaveAR = "معطل"
elseif not redis:get(RAMBOV2.."autoleave") then
autoleaveAR = "معطل"
end
if redis:get(RAMBOV2.."joinbylink") == "On" then
joinbylinkAR = "مفعل"
elseif redis:get(RAMBOV2.."joinbylink") == "Off" then
joinbylinkAR = "معطل"
elseif not redis:get(RAMBOV2.."joinbylink") then
joinbylinkAR = "معطل"
end
...
Uptime_1 = redis:get(RAMBOV2.."bot:Uptime")
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local lockRAMBOV2 = {string.match(text, "^(قفل) (.*)$")}
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيـــــــه فقــــــــط', 1, 'md')
else
if lockRAMBOV2[2] == "التعديل" then
if not redis:get(RAMBOV2.."editmsg" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التعديل \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التعديل مقفول سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2..'editmsg'..msg.chat_id_,'delmsg')
end
if lockRAMBOV2[2] == "الاوامر" then
if not redis:get(RAMBOV2.."bot:cmds" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الاوامر \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الاوامر مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:cmds" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "البوتات" then
if not redis:get(RAMBOV2.."bot:bots:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوتات \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:bots:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "البوتات بالطرد" then
if not redis:get(RAMBOV2.."bot:botskick" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوتات بالطرد \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊البوتات بالطرد مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:botskick" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "التكرار" then
if not redis:get(RAMBOV2.."anti-flood:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التكرار \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التكرار مقفول سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."anti-flood:" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "التثبيت" then
if not redis:get(RAMBOV2.."bot:pin:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التثبيت \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التثبيت مقفول سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:pin:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الكل" then
if not redis:get(RAMBOV2.."bot:muteall" .. msg.chat_id_) then
...
redis:set(RAMBOV2.."bot:bots:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."anti-flood:" .. msg.chat_id_, true)
redis:set(RAMBOV2.."bot:photo:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."bot:video:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."bot:selfvideo:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."bot:document:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."markdown:lock" .. msg.chat_id_, true)
redis:set(RAMBOV2.."bot:gifs:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."bot:music:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."bot:voice:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."tags:lock" .. msg.chat_id_, true)
redis:set(RAMBOV2.."bot:contact:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."bot:tgservice:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."bot:forward:mute" .. msg.chat_id_, true)
redis:set(RAMBOV2.."ffosh"..msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الدردشه" then
if not redis:get(RAMBOV2.."bot:text:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل الدردشه \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الدردشه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:text:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الصور" then
if not redis:get(RAMBOV2.."bot:photo:mute" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "المشاركه" then
if not redis:get(RAMBOV2.."bot:duplipost:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل المشاركه \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ المشاركه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:duplipost:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الكلايش" then
if not redis:get(RAMBOV2.."bot:spam:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل الكلايش \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الكلايش مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:spam:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الفيديو" then
if not redis:get(RAMBOV2.."bot:video:mute" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "السيلفي" then
if not redis:get(RAMBOV2.."bot:selfvideo:mute" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "الفايلات" then
if not redis:get(RAMBOV2.."bot:document:mute" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "الالعاب" then
if not redis:get(RAMBOV2.."Game:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الالعاب \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الالعاب مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."Game:lock" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الانلاين" then
if not redis:get(RAMBOV2.."bot:inline:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانلاين \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانلاين مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:inline:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "البوست" then
if not redis:get(RAMBOV2.."post:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوست \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوست مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."post:lock" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الماركدون" then
if not redis:get(RAMBOV2.."markdown:lock" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "المتحركه" then
if not redis:get(RAMBOV2.."bot:gifs:mute" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "الاغاني" then
if not redis:get(RAMBOV2.."bot:music:mute" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "الصوت" then
if not redis:get(RAMBOV2.."bot:voice:mute" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "الروابط" then
if not redis:get(RAMBOV2.."bot:links:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الروابط \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الروابط مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:links:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "المواقع" then
if not redis:get(RAMBOV2.."bot:location:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل المواقع \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ المواقع مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:location:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "التاك" then
if not redis:get(RAMBOV2.."tags:lock" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "الهاشتاك" then
if not redis:get(RAMBOV2.."bot:hashtag:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الهاشتاك \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الهاشتاك مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:hashtag:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الجهات" then
if not redis:get(RAMBOV2.."bot:contact:mute" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "الويب" then
if not redis:get(RAMBOV2.."bot:webpage:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الويب \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الويب مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:webpage:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "العربيه" then
if not redis:get(RAMBOV2.."bot:arabic:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل العربيه \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ العربيه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:arabic:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الانظمام" then
if not redis:get(RAMBOV2.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانظمام \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانظمام مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:member:lock" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الانكليزيه" then
if not redis:get(RAMBOV2.."bot:english:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانكليزي \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانكليزيه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:english:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "الاشعارات" then
if not redis:get(RAMBOV2.."bot:tgservice:mute" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "الملصقات" then
if not redis:get(RAMBOV2.."bot:sticker:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الملصقات \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الملصقات مقفوله سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOV2.."bot:sticker:mute" .. msg.chat_id_, true)
end
if lockRAMBOV2[2] == "التوجيه" then
if not redis:get(RAMBOV2.."bot:forward:mute" .. msg.chat_id_) then
...
if lockRAMBOV2[2] == "الفشار" then
if not redis:get(RAMBOV2.."ffosh" .. msg.chat_id_) then
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local UNRAMBOV2 = { string.match(text, "^(فتح) (.*)$") }
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيـــــــه فقــــــــط', 1, 'md')
else
if UNRAMBOV2[2] == "التعديل" then
if redis:get(RAMBOV2.."editmsg" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التعديل \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
redis:del(RAMBOV2.."editmsg" .. msg.chat_id_)
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التعديل مفتوح سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
end
if UNRAMBOV2[2] == "الاوامر" then
if redis:get(RAMBOV2.."bot:cmds" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الاوامر \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الاوامر مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:cmds" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "البوتات" then
if redis:get(RAMBOV2.."bot:bots:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوتات \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:bots:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "البوتات بالطرد" then
if redis:get(RAMBOV2.."bot:botskick" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوتات بالطرد \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات بالطرد مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:botskick" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "التكرار" then
if redis:get(RAMBOV2.."anti-flood:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التكرار \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التكرار مفتوح سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."anti-flood:" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "التثبيت" then
if redis:get(RAMBOV2.."bot:pin:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التثبيت \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التثبيت مفتوح سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:pin:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "الكل" then
if redis:get(RAMBOV2.."bot:muteall" .. msg.chat_id_) then
...
redis:del(RAMBOV2.."bot:bots:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."anti-flood:" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:photo:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:video:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:selfvideo:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:document:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."markdown:lock" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:gifs:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:music:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:voice:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."tags:lock" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:contact:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:tgservice:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."bot:forward:mute" .. msg.chat_id_)
redis:del(RAMBOV2.."ffosh"..msg.chat_id_)
end
if UNRAMBOV2[2] == "الدردشه" then
if redis:get(RAMBOV2.."bot:text:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح الدردشه \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الدردشه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:text:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "الصور" then
if redis:get(RAMBOV2.."bot:photo:mute" .. msg.chat_id_) then
...
if UNRAMBOV2[2] == "المشاركه" then
if redis:get(RAMBOV2.."bot:duplipost:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح المشاركه \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ المشاركه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:duplipost:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "الكلايش" then
if redis:get(RAMBOV2.."bot:spam:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح الكلايش \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الكلايش مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:spam:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "الفيديو" then
if redis:get(RAMBOV2.."bot:video:mute" .. msg.chat_id_) then
...
if UNRAMBOV2[2] == "الالعاب" then
if redis:get(RAMBOV2.."Game:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الالعاب \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الالعاب مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."Game:lock" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "الانلاين" then
if redis:get(RAMBOV2.."bot:inline:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانلاين \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانلاين مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:inline:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "البوست" then
if redis:get(RAMBOV2.."post:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوست \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")            
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوست مفتوح سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."post:lock" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "الماركدون" then
if redis:get(RAMBOV2.."markdown:lock" .. msg.chat_id_) then
...
if UNRAMBOV2[2] == "الروابط" then
if redis:get(RAMBOV2.."bot:links:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الروابط \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الروابط مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:links:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "المواقع" then
if redis:get(RAMBOV2.."bot:location:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح المواقع \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ المواقع مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:location:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "التاك" then
if redis:get(RAMBOV2.."tags:lock" .. msg.chat_id_) then
...
if UNRAMBOV2[2] == "الهاشتاك" then
if redis:get(RAMBOV2.."bot:hashtag:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الهاشتاك \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الهاشتاك مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:hashtag:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "الجهات" then
if redis:get(RAMBOV2.."bot:contact:mute" .. msg.chat_id_) then
...
if UNRAMBOV2[2] == "الويب" then
if redis:get(RAMBOV2.."bot:webpage:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الويب \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الويب مفتوح سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:webpage:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "العربيه" then
if redis:get(RAMBOV2.."bot:arabic:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح العربيه \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ العربيه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:arabic:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "الانظمام" then
if redis:get(RAMBOV2.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانظمام \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانظمام مفتوح سابقا\n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:member:lock" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "الانكليزيه" then
if redis:get(RAMBOV2.."bot:english:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانكليزي \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانكليزيه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:english:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "الاشعارات" then
if redis:get(RAMBOV2.."bot:tgservice:mute" .. msg.chat_id_) then
...
if UNRAMBOV2[2] == "الملصقات" then
if redis:get(RAMBOV2.."bot:sticker:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الملصقات \n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الملصقات مفتوحه سابقا\n🎟┊ الأمر بواسطه » "..th3victory(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOV2.."bot:sticker:mute" .. msg.chat_id_)
end
if UNRAMBOV2[2] == "التوجيه" then
if redis:get(RAMBOV2.."bot:forward:mute" .. msg.chat_id_) then
...
if UNRAMBOV2[2] == "الفشار" then
if redis:get(RAMBOV2.."ffosh"..msg.chat_id_) then
...
redis:del(RAMBOV2.."keed_text" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_photo" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_video" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_Document" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_markdon" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_gif" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_audio" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_voice" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_link" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_user" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_hashtag" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_contect" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_arbic" .. msg.chat_id_)
end
...
redis:del(RAMBOV2.."keed_fosh" .. msg.chat_id_)
end
...
redis:set(RAMBOV2.."bot:sens:spam" .. msg.chat_id_, sensspam[2])
...
local Rmbanned = redis:scard(RAMBOV2.."bot:banned:" .. msg.chat_id_) or "0"
redis:del(RAMBOV2.."bot:banned:" .. msg.chat_id_)
...
redis:setex(RAMBOV2.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 35, true)
end end
----------------------------------------------
...
local Rmmomod = redis:scard(RAMBOV2.."bot:momod:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد الأدمنيه ≈ *"..Rmmomod.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(RAMBOV2.."bot:momod:" .. msg.chat_id_)
end end
-----------------------------------------------
...
local Rmowners = redis:scard(RAMBOV2.."bot:owners:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المدراء ≈ *"..Rmowners.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(RAMBOV2.."bot:owners:" .. msg.chat_id_)
end end
-----------------------------------------------
...
local Rmmonshis = redis:scard(RAMBOV2.."bot:monshis:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المنشئين ≈ *"..Rmmonshis.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(RAMBOV2.."bot:monshis:" .. msg.chat_id_)
end end
--------DEL RAMBOV2_SUDO-----------------------------------------
...
local Rmvipmem = redis:scard(RAMBOV2.."bot:vipmem:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المميزين ≈ *"..Rmvipmem.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(RAMBOV2.."bot:vipmem:" .. msg.chat_id_)
end  end
---------------keeed del --------------------------------------------
...
local Rmkeed = redis:scard(RAMBOV2.."bot:keed:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المقيدين ≈ *"..Rmkeed.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
local hash = "bot:keed:" .. msg.chat_id_
local list = redis:smembers(RAMBOV2..hash)
for k, v in pairs(list) do
redis:del(RAMBOV2.."bot:keed:" .. msg.chat_id_)
HTTPS.request("https://api.telegram.org/bot"..RAMBOV2_TOKEN.."/restrictChatMember?chat_id=" ..msg.chat_id_.. "&user_id=" ..v.."&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
end end end
------------DEL vipmems-------------------------------------------
...
local Rmmuted = redis:scard(RAMBOV2.."bot:muted:" .. msg.chat_id_) or "0"
redis:del(RAMBOV2.."bot:muted:" .. msg.chat_id_)
...
if redis:get(RAMBOV2.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local d = 0
if text:match("^1$") then
redis:del(RAMBOV2.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
for i = 1, NumberReturn do
getChannelMembers(msg.chat_id_, d, "Kicked", 200, kickedlist)
d = d + 200
end
if redis:get(RAMBOV2.."lang:gp:" .. msg.chat_id_) then
...
if not redis:get(RAMBOV2.."flood:max:" .. msg.chat_id_) then
flood_m = 5
else
flood_m = redis:get(RAMBOV2.."flood:max:" .. msg.chat_id_)
end
if not redis:get(RAMBOV2.."bot:sens:spam" .. msg.chat_id_) then
spam_c = 400
else
spam_c = redis:get(RAMBOV2.."bot:sens:spam" .. msg.chat_id_)
end
if redis:get(RAMBOV2.."warn:max:" .. msg.chat_id_) then
sencwarn = tonumber(redis:get(RAMBOV2.."warn:max:" .. msg.chat_id_))
else
sencwarn = 4
end
if redis:get(RAMBOV2.."floodstatus" .. msg.chat_id_) == "DelMsg" then
floodstatus = "المسح"
elseif redis:get(RAMBOV2.."floodstatus" .. msg.chat_id_) == "Kicked" then
floodstatus = "الطرد"
elseif not redis:get(RAMBOV2.."floodstatus" .. msg.chat_id_) then
floodstatus = "الطرد"
end
if redis:get(RAMBOV2.."warnstatus" .. msg.chat_id_) == "Muteuser" then
warnstatus = "الكتم"
elseif redis:get(RAMBOV2.."warnstatus" .. msg.chat_id_) == "Remove" then
warnstatus = "الطرد"
elseif not redis:get(RAMBOV2.."warnstatus" .. msg.chat_id_) then
warnstatus = "الكتم"
end
if redis:get(RAMBOV2.."bot:music:mute" .. msg.chat_id_) then
mute_music = "✅"
...
if redis:get(RAMBOV2.."bot:welcome" .. msg.chat_id_) then
send_welcome = "✅"
else
send_welcome = "✖️"
end
...
os.execute('wget https://raw.githubusercontent.com/rami009/rrr/master/RAMBOV2.lua') 
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم تحديث السورس \n ✓ ", 1, 'md')
dofile('RAMBOV2.lua')  
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
redis:setex(RAMBOV2.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
redis:set(RAMBOV2..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if redis:get(RAMBOV2.."lang:gp:" .. result.id_) then
send(result.id_, 0, 1, "🌀┊  تم اخراج البوت 🎐\n🔱┊ راسل المطور للتفعيل 📮\n🔰┊ *المطور* : [" .. UserRAMBOV2 .. "]\n‏", 1, "md")
else
send(result.id_, 0, 1, "🌀┊  تم اخراج البوت 🎐\n🔱┊ راسل المطور للتفعيل 📮\n🔰┊ *المطور* : [" .. UserRAMBOV2 .. "]\n‏", 1, "md")
end
chat_leave(result.id_, bot_id)
redis:srem(RAMBOV2.."bot:groups", result.id_)
else
...
if redis:get(RAMBOV2.."bot:reloadingtime") then
send(msg.chat_id_, msg.id_, 1, "🌀┊  تم تحديث البوت 🎐 ", 1, "md")
else
getUser(txt[2], get_data)
end end end
----------INFO RAMBOV2_SUDO BY REPLY----------------------------------------------------------------------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local rami =  text:match("^اذاعه خاص (.*)")
local rami2 = "🌀┊ تم ارسال الاذاعه الى:\n( * GP * ) من الاعضاء\n‏"
local gp = tonumber(redis:scard(RAMBOV2.."bot:userss"))
gps = redis:smembers(RAMBOV2.."bot:userss")
text = rami2:gsub('GP',gp)
for k,v in pairs(gps) do
send(v, 0, 1,rami, 1, 'md')
end
send(msg.chat_id_, msg.id_, 1,text, 1, 'md')
end end end
----------------promote_admin------------------
...
if not redis:get(RAMBOV2.."dellmssg"..msg.chat_id_) and not is_owner(msg.sender_user_id_, msg.chat_id_) then
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
if not redis:get(RAMBOV2.."lock_pinn"..msg.chat_id_) and not is_owner(msg.sender_user_id_, msg.chat_id_) then
...
redis:set(RAMBOV2.."pinnedmsg" .. msg.chat_id_, msg.reply_to_message_id_)
...
function gp_RAMBOV2_info(arg,data)
local list = redis:smembers(RAMBOV2.."bot:owners:" .. msg.chat_id_)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(RAMBOV2.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم مسح الرد بنجاح\n ✓ ", 1, 'md')
redis:del(RAMBOV2..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(RAMBOV2..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(RAMBOV2..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(RAMBOV2..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(RAMBOV2..'video_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(RAMBOV2..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(RAMBOV2..'rep_owner'..msg.content_.text_..''..msg.chat_id_..'')
return false
end
end
--------------------------------------------------------------------------
if msg.content_.text_ == 'اضف رد' and is_owner(msg.sender_user_id_, msg.chat_id_)  then
redis:set(RAMBOV2..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','set_repgp')
send(msg.chat_id_, msg.id_, 1, '📌┊ ارسل لي كلمه الرد الان ❗️\n',1, 'md')
return false    end
if msg.content_.text_ then
local content_keep = redis:get(RAMBOV2..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
if content_keep == 'set_repgp' then
send(msg.chat_id_, msg.id_, 1, '📌┊ ارسل جواب الرد قد يكون\n🗯┊ {نص-ملصق-بصمه...} ❗️\n' ,  1, 'md')
redis:set(RAMBOV2..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','save_repgp')
redis:set(RAMBOV2..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'',msg.content_.text_)
redis:sadd(RAMBOV2..'rep_owner'..msg.chat_id_..'',msg.content_.text_)
return false
end
end
-------------------------------------------------------------------------
if  msg.content_.text_ == 'الردود' and is_owner(msg.sender_user_id_, msg.chat_id_) then
local redod = redis:smembers(RAMBOV2..'rep_owner'..msg.chat_id_..'')
if #redod == 0 then
...
for k,v in pairs(redod) do
redis:del(RAMBOV2..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(RAMBOV2..'gif_repgp'..v..msg.chat_id_)
redis:del(RAMBOV2..'voice_repgp'..v..msg.chat_id_)
redis:del(RAMBOV2..'stecker_repgp'..v..msg.chat_id_)
redis:del(RAMBOV2..'video_repgp'..v..msg.chat_id_)
redis:del(RAMBOV2..'text_repgp'..v..msg.chat_id_)
redis:del(RAMBOV2..'rep_owner'..msg.chat_id_..'',msg.content_.text_)
end
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم مسح جميع الردود\n ✓ ", 1, 'md')
return false
end
...
redis:set(RAMBOV2.."add:repallt"..msg.sender_user_id_,'del_rep1')
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل لي كلمه الرد لمسحها ❗️\n",1, "md")
return false
end
if msg.content_.text_ then
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم مسح الرد للكل\n ✓ ", 1, 'md')
redis:del(RAMBOV2.."add:repallt"..msg.sender_user_id_)
redis:del(RAMBOV2.."gif_repall"..msg.content_.text_)
redis:del(RAMBOV2.."voice_repall"..msg.content_.text_)
redis:del(RAMBOV2.."stecker_repall"..msg.content_.text_)
redis:del(RAMBOV2.."video_repall"..msg.content_.text_)
redis:del(RAMBOV2.."text_repall"..msg.content_.text_)
redis:del(RAMBOV2.."rep_sudo",msg.content_.text_)
return false
end
end
--------------------------------------------------------------------------
if msg.content_.text_ == 'اضف رد للكل' and is_sudo(msg)  then
redis:set(RAMBOV2.."add:repallt"..msg.sender_user_id_,'set_rep')
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل لي كلمه الرد ❗️\n",1, "md")
return false    end
if msg.content_.text_ then
local content_keep = redis:get(RAMBOV2.."add:repallt"..msg.sender_user_id_)
if content_keep == 'set_rep' then
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل جواب الرد قد يكون\n🗯┊ {نص-ملصق-بصمه...} ❗️\n" ,  1, "md")
redis:set(RAMBOV2.."add:repallt"..msg.sender_user_id_,'save_rep')
redis:set(RAMBOV2.."addreply2:"..msg.sender_user_id_, msg.content_.text_)
redis:sadd(RAMBOV2.."rep_sudo",msg.content_.text_)
return false
end    end
------------------------------------------------------------------------------------
if  msg.content_.text_ == "ردود المطور" and is_sudo(msg) then
local redod = redis:smembers(RAMBOV2.."rep_sudo")
if #redod == 0 then
...
for k,v in pairs(redod) do
redis:del(RAMBOV2.."add:repallt"..v)
redis:del(RAMBOV2.."gif_repall"..v)
redis:del(RAMBOV2.."voice_repall"..v)
redis:del(RAMBOV2.."stecker_repall"..v)
redis:del(RAMBOV2.."video_repall"..v)
redis:del(RAMBOV2.."text_repall"..v)
redis:del(RAMBOV2.."rep_sudo",msg.content_.text_)
end
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..th3victory(msg).."\n🎟┊ تم مسح ردود المطور\n ✓ ", 1, 'md')
return false
...
local list = redis:smembers(RAMBOV2.."bot:owners:" .. chattid)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(RAMBOV2.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
if user_info then
...
local Rmmonshis = redis:scard(RAMBOV2.."bot:monshis:" .. chattid) or "0"
local Rmbanned = redis:scard(RAMBOV2.."bot:banned:" .. chattid) or "0"
local Rmowners = redis:scard(RAMBOV2.."bot:owners:" .. chattid) or "0"
local Rmmuted = redis:scard(RAMBOV2.."bot:muted:" .. chattid) or "0"
local Rmkeed = redis:scard(RAMBOV2.."bot:keed:" .. chattid) or "0"
local Rmmomod = redis:scard(RAMBOV2.."bot:momod:" .. chattid) or "0"
local Rmvipmem = redis:scard(RAMBOV2.."bot:vipmem:" .. chattid) or "0"
if not redis:get(RAMBOV2.."bot:group:link"..chattid) then
local getlink = 'https://api.telegram.org/bot'..RAMBOV2_TOKEN..'/exportChatInviteLink?chat_id='..chattid
local req = https.request(getlink)
local link = RmJS:decode(req)
if link.ok == true then 
redis:set(RAMBOV2.."bot:group:link"..chattid,link.result)
end
end
local lik_1 = redis:get(RAMBOV2.."bot:group:link"..chattid)
if lik_1 then
link = lik_1 
else
link = link.result
end
send(msg.chat_id_, msg.id_, 1, "🕴┊ المدير » ["..owner.."]\n🥈┊ﮧ ["..title_name(chattid).."]("..(link or "t.me/RAMBOV2_ch")..")\n©️┊ عدد المــــدراء   » *"..Rmowners.."*\n©️┊ عدد المنشئين   » *"..Rmmonshis.."*\n🔆┊ عدد الادمنيـــه   » *"..Rmmomod.."*\n🔰┊ عدد المكتومين  » *"..Rmmuted.."*\n®️┊ عدد المحظورين » *"..Rmbanned.."*\n®️┊ عدد المقيديــن   » *"..Rmkeed.."*\n🗯┊ عدد المميزيـــن  » *"..Rmvipmem.."*\n💠┊ﮧ `"..chattid.."`\n🏮┊ المطور » ["..sudo.."]\n✓", 1,"md")
end
tdcli_function ({
ID = "GetChannelFull",
...
getChannelFull(chattid, gp_RAMBOV2_info, nil)
end 
end
------------UNPIN-----------------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
tdcli.sendDocument(RAMBOV2_SUDO, 0, 0, 1, nil, './RAMBOV2.lua', dl_cb, nil)
...
pinmsg(msg.chat_id_, pin_id, 0)
elseif redis:get(RAMBOV2.."lang:gp:" .. msg.chat_id_) then
...
if not redis:get(RAMBOV2.."lock_kickme"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "🌀┊ امر اطردني معطل 🍃", 1, "md")
return false
end
redis:set(RAMBOV2.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:set(RAMBOV2.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
send(msg.chat_id_, msg.id_, 1, "🔰┊ ارسل *{ نعم }* لطردك\n🚫┊ ارسل *{ لا } *لالغاء طردك", 1, "md")
end
local Rmkick = redis:get(RAMBOV2.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
...
if Rmkick == "kick_mee" then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del(RAMBOV2.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:del(RAMBOV2.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
send(msg.chat_id_, msg.id_, 1, "🔰┊ تم طردتك حياتي", 1, "md")
end 
end
end
if text:match("^لا$") then
local Rmunkick = redis:get(RAMBOV2.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
redis:set(RAMBOV2.."bot:viewget" .. msg.sender_user_id_, true)
...
redis:set(RAMBOV2.."addstart1"..msg.sender_user_id_, "theaddstarts")
return false
end end
if text then
local theaddstart = redis:get(RAMBOV2.."addstart1"..msg.sender_user_id_)
if theaddstart == 'theaddstarts' then
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم حفظ الكليشه 🍃", 1, 'md')
redis:del(RAMBOV2.."addstart1"..msg.sender_user_id_)
redis:set(RAMBOV2.."startbot", text)
return false
end end
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local get_me = function(extra, result)
local msgs = (tonumber(redis:get(RAMBOV2.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_)) or "0" )
local Rmtext = (tonumber(redis:get(RAMBOV2.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local Rmsticker = (tonumber(redis:get(RAMBOV2.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local pRmhoto = (tonumber(redis:get(RAMBOV2.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local RmVoice = (tonumber(redis:get(RAMBOV2.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local RmGif = (tonumber(redis:get(RAMBOV2.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local RmVideo = (tonumber(redis:get(RAMBOV2.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local RmSelfVideo = (tonumber(redis:get(RAMBOV2.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local Rmcon = (tonumber(redis:get(RAMBOV2.."Rmaddcon"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
if result.first_name_ then
if #result.first_name_ < 25 then
else
for kkkkk in string.gmatch(result.first_name_, "[^%s]+") do
...
--------------------------------------------------------
if text == ''..(redis:get(RAMBOV2..'RAMBOV2nams') or 'كيبر')..' هينه' or text == ''..(redis:get(RAMBOV2..'RAMBOV2nams') or 'كيبر')..' رزله' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(RAMBOV2_SUDO) then
send(msg.chat_id_, msg.id_, 1, 'انجب لك هذا مطوري العشق 😌💋', 1, 'md')
return false  end
if tonumber(result.sender_user_id_) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, 1, 'لك مگدر اهين نفسي 😞😂', 1, 'md')
return false  end
local RAMBOV2 = { "لك دايح ، احترم نفسك لا بال 👠😠","ها مصراع تراچي ، اگعد راحه تره روحي طالعه 😐🍃","ها ابن الحنينه، ليش متسكت وتنجب 🌚"}
send(msg.chat_id_, result.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
...
--------------------------------------------------------------------------------------------
if text == ''..(redis:get(RAMBOV2..'RAMBOV2nams') or 'كيبر')..' بوسه' or text == ''..(redis:get(RAMBOV2..'RAMBOV2nams') or 'كيبر')..' مصه' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(RAMBOV2_SUDO) then
...
return false  end
local RAMBOV2 = { "اععع 🤢خده بي حب شباب الوصخ😹😹","موااح 💋 مواااح  حياتي💋😌🍃","💋😞نسخ لصق ع الشفه 👄"}
send(msg.chat_id_, result.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
if tonumber(msg.reply_to_message_id_) == 0 then
else
...
--------------------------------- rdood bot --------------------------------------------------
if not redis:get(RAMBOV2.."lock_reeeep"..msg.chat_id_) then
if text == "السلام عليكم" or text == "سلام عليكم" or text == "سلام" then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"وعليكم السلام والرحمه⇣😻","يمه هلا بالغالي 😻🍃","وعليكم السلام حبيبي ☺️🍃","كافي بس تسلمون 🌝💔" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {'اسمي '..(redis:get(RAMBOV2..'RAMBOV2nams') or 'كيبر')..' 😌👌','تره اسمي '..(redis:get(RAMBOV2..'RAMBOV2nams') or 'كيبر')..' يالجريذي ☹️😹'  }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
if text == ''..(redis:get(RAMBOV2..'RAMBOV2nams') or 'كيبر')..'' then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {'اي نعم اني '..(redis:get(RAMBOV2..'RAMBOV2nams') or 'كيبر')..' 🍃😐','اهو اجوي الملطلطين 😹😪','كافي تره كرهت اسمي 💔😠'}
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-------------------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"هلووووات  ⁽🙆♂✨₎ֆ","يمه هلا بالعافيه 😻🍃","لا هلا ولا مرحبه شلونك مشتاقين 😻😂","اخلاً وصخلا 😌😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"تمام وانت/ي 😘🍃","شعليك انت 🧐😂","بخير انت/ي شلونك/ج ☺️","تمام وانت/ي ‏ ᵛ͢ᵎᵖ💛﴾" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"عـٰٰـٰود لوتُٰي 🙀 يـٰگول بُِوت عبالـٰه طافٰـُٰي💔 ويضِٰل يمٰـٓسلت وينشٰٰر روابـٰٓط 😳🍃","اي بوت شتريد 😤" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"شكلولك عليه كاولي 😶😂","صوتي محلو للاسف 😌💔","اشعجـب كاطع بيه ياراحتي النفسيه 😂💔","حرام  الغنا 😐🍃" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"بايات 💛🍃","گلعه 😶💔","الله الله الله وياك 😻😂","ثيمالا 🌝✋🏾" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"منو ال أجوو👀😹","جااااوات  ₎✿💥😈 ❥" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"نورك/ج هذا ورده 🌝🍃","بوجودك/غلا تسلم 😻✨","انت/ي اصل النور 😋🍃","عماني نورك 😣😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "بوسني" or text == "بوسه" then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then local RAMBOV2 = {"مووووووووواححح💋😻","مابوس ولي😌😹","خدك/ج نضيف 😂🍃","البوسه بالف حمبي 🌝💋" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"عيـــب 🙀😹","شبيه امك حمبي😋🍃" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"شبيه الشكره ام الوصخ 🤭😹","حبيته فدوووه😻","شرايد من خالته 🤭😂","خالته تفلش 😶😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"تف عليك ادبسزز 😒😹","لا تتفل على وجهك 😻😹","ما اسمحلك هيلگ 😡😹","بدون تفال رجائاً 😹😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"كافـي ضحــك 😐","لتضحك هواي وتصير فاول 🌝😹","هذ شبي يضحك 🙀😳","اضحك هيه الدنيا خربانه 😂😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"لصير عصبي يرتفع ضغطـك 😌😂","صار وجه احمر مثل الطماطه 🙊😹","اوف شحلاتك وانت ضايج 😻","شبي هذا الله يستر 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😒" or text == "😒😒" or text == "😒😒😒" or text == "😒😒😒😒" then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"لضوج متسوووه 🤭😹","يمه زعلان الحلو ما يكلي مرحبا 😻😹","اعدل وجهك لا اعدله الك/ج 😼👊🏼" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"يمه الگمر عذبني حبه 🙊😻","عو نضيف الوصخ 😹😹","طفي ضواك عميتني 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "💋" or text == "💋💋" then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"كبر 🙀 جان استحيتو 😹😹","عســـل 😋🙊" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "💔" or text == "💔💔" then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"شبي مكسور 😔💔","موجوع كلبي والتعب بيه 😔😹","اكل بصل وانسه الحصل 😻😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😻" or text == "😻😻" or text == "😍😍" or text == "😍" then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"شوفو الحب صاعد فول 😻😹","ها ناوي تزحف 😹😹","فدوووه لهاي العيون 🙊😻" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😐" or text == "🙂" then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"شبيك حبيبي 😂💔","منور محمد الاعمى 😐😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"دووم الضحكه 🙊🍃","دوم الضحكه ℡̮⇣┆👑😻⇣ۦ ٰ" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"اي غرد/ي دا اسمع 👂🏽😹","كول😹 (كول لو هدف)😔😹","ها حياتي 🙊" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"انت/ي شبيك/ج😣","مابيه شي تسلم 💋😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
----------------------------------------
if text == "🌚💔" or text == "💔" then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"هم اجانه محترك وجه😂♥️","هاي منو كاسر كلبك😡","اهو هم اجانه صخام🐸👌" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
---------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"ها بدت حبجيه ✨😂","لتلح عود يعني احبج🙈😹","كافي درينه مشتاقله 😒" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
---------------------------------------
if text == "😢" or text == "😢😢" then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"يبجي دلوع😜😹","هاي عود انت جبير كاعد تبجي😑💔","لتلح درينه تبجي😒"}
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
--------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"علساس انت متفاعل😒😒","اي عندك اعتراض🤔","اني معليه احرسكم😎" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"عود اني حباب ونت شيطان يتعلم منك🙈😂😂","وجهك ميساعد🤢😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"اهو لا هلا بيه ✨😂","خي ولي مزاعله ✨😂","اهلا بيه بس اطرده اذا اجه😒😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
------------------------------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"وخرو وخرو🤓 هلا بلخميس تيرارا وياي يلا😍😹","هلا بلخميس عطله وكذا ركصو يلا😍😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "🙊" or text == "🙈" then
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"اول مره اشوف قرد يستحي🤔😂","ما مرجيه منك هايه صاير تستحي انته هوايه 😍😂😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
--------------------------------------
...
if not redis:get(RAMBOV2..'lock:add'..msg.chat_id_) then
local RAMBOV2 = {"ها ها يمعودين احنه هنا😒😹","اعتقد اكو خاص وخطبو وهنا زفه بسيارتي🚗😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOV2[math.random(#RAMBOV2)]..'', 1, 'md')
end
end
end
...
if idf:match("-100(%d+)") and not redis:get(RAMBOV2.."bot:muted:Time" .. chat .. ":" .. user) and redis:sismember(RAMBOV2.."bot:muted:" .. chat, user) then
redis:srem(RAMBOV2.."bot:muted:" .. chat, user)
end
...
if (text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]")) and redis:get(RAMBOV2.."bot:links:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if result.content_.entities_ and result.content_.entities_[0] and (result.content_.entities_[0].ID == "MessageEntityTextUrl" or result.content_.entities_[0].ID == "MessageEntityUrl") and redis:get(RAMBOV2.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if result.content_.web_page_ and redis:get(RAMBOV2.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if (text:match("[Hh][Tt][Tt][Pp]") or text:match("[Ww][Ww][Ww]") or text:match(".[Cc][Oo]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Rr]")) and redis:get(RAMBOV2.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("@") and redis:get(RAMBOV2.."tags:lock" .. msg.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("#") and redis:get(RAMBOV2.."bot:hashtag:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("[\216-\219][\128-\191]") and redis:get(RAMBOV2.."bot:arabic:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
...
if redis:get(RAMBOV2..'bot:english:mute'..result.chat_id_) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs)
end
end
if redis:get(RAMBOV2..'editmsg'..msg.chat_id_) == 'delmsg' then
local id = msg.message_id_
local msgs = {[0] = id}
local chat = msg.chat_id_
delete_msg(chat,msgs)
elseif redis:get(RAMBOV2..'editmsg'..msg.chat_id_) == 'didam' then
if redis:get(RAMBOV2..'bot:editid'..msg.message_id_) then
local old_text = redis:get(RAMBOV2..'bot:editid'..msg.message_id_)
...
redis:sadd(RAMBOV2.."groups:users" .. msg.chat_id_, msg.sender_user_id_)
if text then
...
redis:set(RAMBOV2.."Bot:RmBotAccount", data.value_.value_)
end end end end  end
----- End Source By Karrar RAMBOV2 »»»»»»»»»»»»»»»»»»»
